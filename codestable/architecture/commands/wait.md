...
- `verify.ts` 调用 `managedVerify`，先解析 assertion，再对 URL 或 target 做 read-only assertion；`passed=false` 时由 engine 写入 `VERIFY_FAILED` run event，供 diagnostics bundle 归因。
...
- `VERIFY_FAILED`：断言完成但不通过；恢复是 `read-text`、`locate`、`snapshot -i`、`page current`，必要时 `diagnostics bundle`。失败会进入 run artifact，因此 bundle 可识别 `failureKind=VERIFY_FAILED`。
...
| `verify` | `proven` | 2026-05-04 automated-testing workflow 覆盖正向断言和 `VERIFY_FAILED` bundle 归因；wait/assert focused check 覆盖断言矩阵 |
...
- `verify` 失败返回 `VERIFY_FAILED`，不伪装成 action failure，但必须进入 run artifact，保证 diagnostics bundle 能交接失败断言。
