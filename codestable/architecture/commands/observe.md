...
| `page list` | `documented` | skill command reference 记录 |
| `page dialogs` | `documented` | skill command reference 记录 |
| `page assess` | `documented` | skill/failure recovery 记录 inference-only 边界 |
| `snapshot` | `proven` | dogfood 登录页 snapshot 和 benchmark perception 覆盖 |
| `snapshot status` | `documented` | skill/failure recovery 记录 ref epoch 检查 |
| `read-text` | `proven` | dogfood、benchmark perception 和 workflow 主链覆盖 |
| `text` | `documented` | skill 记录为 `read-text` 短别名 |
| `screenshot` | `proven` | benchmark generated perception/diagnostics task 允许并记录截图证据 |
| `pdf` | `documented` | skill command reference 记录低频归档用途 |
| `accessibility` | `documented` | skill command reference 和 domain-status 记录 |

**状态分布：** proven 6 / documented 8 / experimental 0
