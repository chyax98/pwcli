...
| `fill` | `proven` | dogfood 登录表单覆盖，formdog 验证 `fill` + `get value` |
...
| `hover` | `proven` | controldog 覆盖 selector hover 和 action evidence |
| `check` | `proven` | formdog 登录表单和 deterministic fixture 覆盖 |
| `uncheck` | `proven` | formdog 登录表单覆盖 |
| `select` | `proven` | deterministic fixture 覆盖 selector + value 选择 |
| `scroll` | `proven` | controldog 覆盖 scroll down 后文本等待 |
| `resize` | `proven` | controldog 覆盖 iphone preset 和 viewport 复查 |
| `mouse` | `proven` | controldog/mousedog2 覆盖 coordinate move/click 和页面状态复查 |
| `dialog` | `proven` | dogfood modal/dialog recovery 覆盖 |

**状态分布：** proven 15 / documented 0 / experimental 0
