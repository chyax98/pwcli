...
| `auth dc` | `documented` | skill 和 Forge/DC 专项文档记录；真实 Forge/DC 不在 dogfood 覆盖 |
| `state save` | `proven` | dogfood 保存登录态覆盖 |
| `state load` | `proven` | dogfood 新 session 复用 state 覆盖 |
| `state diff` | `proven` | statedog 覆盖 baseline 创建和 before/after cookies/localStorage diff |
...
**状态分布：** proven 11 / documented 2 / experimental 0
