...
| `auth list/info` | provider name 来自 positional；`auth info --verbose` 才包含 provider source |
...
- `auth.ts` 从 `#auth/registry.js` 读取 built-in providers；provider 通过 `managedRunCode` 在现有 page 上执行；`auth info` 默认输出 provider 参数、示例和 notes，不输出大段 `source`，需要调试 provider 实现时显式传 `--verbose`。
