...
| 命令 | 状态 | 依据 |
|---|---|---|
| `auth probe` | `proven` | benchmark generated auth-state tasks 覆盖 |
| `auth list/info/dc/fixture-auth` | `documented` | skill 和 Forge/DC 专项文档记录；真实 Forge/DC 不在 dogfood 覆盖 |
| `state save` | `proven` | dogfood 保存登录态覆盖 |
| `state load` | `proven` | dogfood 新 session 复用 state 覆盖 |
| `state diff` | `documented` | skill/failure recovery 记录 |
| `storage local` | `proven` | dogfood localStorage read 覆盖 |
| `storage session` | `proven` | dogfood sessionStorage read 覆盖 |
| `storage indexeddb export` | `documented` | skill command reference 记录 |
| `cookies list` | `proven` | dogfood cookies list 覆盖 |
| `cookies set` | `proven` | dogfood cookies set 覆盖 |
| `bootstrap apply` | `proven` | dogfood bootstrap init script/headers 覆盖 |
| `profile list-chrome` | `documented` | skill command reference 记录 |

**状态分布：** proven 7 / documented 5 / experimental 0
