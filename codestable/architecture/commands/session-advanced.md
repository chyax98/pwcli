...
| `auth dc` | `blocked` | 2026-05-04 真实环境 proof 未通过：默认 local-ip 入口 `DC_AUTH_URL_UNREACHABLE`，显式 `targetUrl` 触发 `RUN_CODE_TIMEOUT` 且后续 session probe 不可读；blocker 见 `codestable/issues/2026-05-04-auth-dc-real-env-proof-blocked/auth-dc-real-env-proof-blocked-report.md` |
