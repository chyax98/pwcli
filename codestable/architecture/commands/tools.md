...
| `geolocation set` | positional latitude/longitude、`--accuracy <m>` |
...
| `dashboard open` | `--dry-run`、`--timeout <ms>`、`--output <text|json>` |
...
- 当前 `src/cli/commands/route.ts` 与 `node dist/cli.js route --help` 只注册 `add|remove|list`；skill 中仍出现的 `route load` 不属于当前 help/source 命令面。
- `ENVIRONMENT_LIMITATION`：environment mutation 在 run-code lane 没完成；换 fresh session 或降级为 unsupported。
- `CLOCK_REQUIRES_INSTALL`：`clock set/resume` 前必须先 `clock install`。
...
| 命令 | 状态 | 依据 |
|---|---|---|
| `batch` | `proven` | dogfood batch file 覆盖；skill 强调 stable `string[][]` 子集 |
| `code` | `proven` | dogfood bootstrap/clock verification 使用 `pw code --file` |
| `route add/remove/list` | `proven` | dogfood route direct、inject、match-body、patch、list/remove 覆盖 |
| `sse` | `documented` | diagnostics skill 记录 SSE 捕获和查询 |
| `environment offline/geolocation/permissions/clock` | `proven` | dogfood environment controls 覆盖 |
| `doctor` | `proven` | dogfood modal blockage recovery 使用 `doctor --session --endpoint` |
| `profile list-chrome` | `documented` | skill 记录为 system Chrome profile discovery |
| `skill path/install` | `documented` | release-v0.1.0 和 skill reference 记录 |
| `dashboard open` | `documented` | skill/failure recovery 记录 dashboard unavailable/launch failed 分支 |

**状态分布：** proven 5 / documented 4 / experimental 0
