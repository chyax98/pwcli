...
- `code.ts` 调用 `managedRunCode`，source 来自 positional 或 `--file`；带 timeout 的 managed session command 使用可主动关闭 socket 的路径，确保 `RUN_CODE_TIMEOUT` 后 CLI 进程及时退出。
...
- `doctor.ts` 组合环境、浏览器安装、磁盘、profile/state/endpoint/auth-provider/session probes；session probe 成功时会把 visible HTML modal/overlay 提升为 `html-modal` recovery，browser dialog blocked state 仍是 `modal-state`。
...
- `RUN_CODE_TIMEOUT`：`code` 或 run-code-backed 长流程超过 guard timeout；CLI 必须及时退出，session 应可继续 `page current` / `status` / `diagnostics digest`，业务流程拆成一等命令和显式 `wait`。
...
- `doctor` 是健康检查，不修复环境；`--verbose` 只展开 probes。`modal-state` 对应 browser dialog，用 `dialog accept|dismiss`；`html-modal` 对应页面内 overlay，用页面按钮恢复。
...
| `code` | `proven` | 2026-05-04 route/mock/bootstrap focused check 覆盖 inline、`--file` 和 `--retry`；`check:run-code-timeout` 覆盖 timeout 后 CLI 及时退出和恢复链路 |
...
| `sse` | `proven` | `test/integration/sse-observation.test.ts` 覆盖 EventSource connect/open/message 捕获；network-console focused check 覆盖 `pw sse --url` 查询 |
...
| `doctor` | `proven` | dogfood modal blockage recovery 使用 `doctor --session --endpoint`；`check:doctor-modal` 固化 browser dialog `modal-state`、confirm dismiss、prompt accept 和 HTML modal `html-modal` recovery |
