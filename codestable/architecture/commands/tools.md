...
| `geolocation set` | `--lat <lat>`、`--lng <lng>`、`--accuracy <m>`；保留 positional latitude/longitude fallback |
...
| `environment offline/geolocation/permissions/clock` | `proven` | dogfood environment controls 覆盖；`check:env-geolocation` 覆盖 `--lat/--lng` 和 positional fallback |
