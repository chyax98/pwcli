...
| 命令 | 状态 | 依据 |
|---|---|---|
| `batch` | `proven` | dogfood batch file 覆盖；skill 强调 stable `string[][]` 子集；`check:batch-verify` 覆盖 verify failure 传播 |
...
| `environment offline/geolocation/permissions/clock` | `proven` | dogfood environment controls 覆盖；`check:env-geolocation` 覆盖 `--lat/--lng` 和旧 positional 形态拒绝 |
