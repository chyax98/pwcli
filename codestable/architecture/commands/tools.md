...
| `sse` | `proven` | ssedog 覆盖 EventSource connect/open/message 捕获和 `pw sse --url` 查询 |
...
| `skill path/install` | `proven` | tool dogfood 暴露路径 P1 并修复；`check:skill-install` 固化 |
| `dashboard open` | `proven` | local dogfood 覆盖 `dashboard open --dry-run` bundled entrypoint 检查 |
