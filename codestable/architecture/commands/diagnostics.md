...
| `pw har start` | HAR 热录制明确失败 guard，返回 `UNSUPPORTED_HAR_CAPTURE` |
| `pw har stop` | HAR 热录制明确失败 guard，返回 `UNSUPPORTED_HAR_CAPTURE` |
...
| `bundle` | `--out <dir>`、`--task <text>`、`--limit <n>`，默认 20 |
...
| `har start` | positional path 或 `--path <path>`；当前只用于返回明确失败 envelope |
| `har replay` | positional HAR file |
| `har replay-stop` | `--session <name>` |
| `video start` | `--session <name>` |
| `video stop` | `--session <name>` |

## 技术原理

- `diagnostics.ts` 基于 `managedDiagnosticsExport`，再用 filter、session digest、run digest、bundle、run view、timeline helper 做投影；`export --out` 写过滤后的 JSON data，`bundle --out` 写 `manifest.json` 和 `handoff.md`。
...
- `har.ts` 中 `managedHar(start|stop)` 只作为明确失败 guard，返回 `UNSUPPORTED_HAR_CAPTURE`；`managedHarReplay` / `managedHarReplayStop` 负责预录制 HAR 回放。
...
- `diagnostics bundle` 的 manifest 是 1.0 稳定证据 contract：`schemaVersion/session/createdAt/task?/commands/runIds/artifacts/summary` 是 Agent 交接和 bug 复现入口；旧的 `auditConclusion/digest/diagnostics/latestRunEvents/timeline/highSignalTimeline` 仍是同一 manifest 内的诊断细节，不另起第二套实现。
- `summary.status` 从诊断归因收敛为 `pass | fail | blocked`：无强失败信号为 `pass`，有失败或高风险信号为 `fail`，`MODAL_STATE_BLOCKED` 类 browser dialog 阻塞为 `blocked`。
...
- `UNSUPPORTED_HAR_CAPTURE`：`har start|stop` 不是 1.0 稳定录制能力。Playwright HAR capture 需要在 BrowserContext 创建时配置，当前 managed session 不做热录制补丁。稳定证据优先 `network`、`diagnostics export|bundle` 和 `trace inspect`。
...
| 命令 | 状态 | 依据 |
|---|---|---|
| `diagnostics digest/export/bundle/runs/show/grep/timeline` | `proven` | diagnostics focused check 覆盖 digest/export/bundle/runs/show/grep/timeline；2026-05-04 evidence bundle 1.0 focused test 覆盖 manifest 1.0、handoff.md、run evidence 和 screenshot artifact metadata |
...
| `har start|stop|replay|replay-stop` | `proven` | 2026-05-04 `check:har-1-0` 证明 start/stop 明确返回 `UNSUPPORTED_HAR_CAPTURE`，预录制 HAR replay/replay-stop 可执行并能拦截页面请求 |
| `video start|stop` | `proven` | 2026-05-04 artifact focused check 覆盖 video start/stop 和非空 WebM artifact 输出 |

**状态分布：** proven 8 / documented 0 / experimental 0
...
- HAR replay 是 mock/diagnostics 的补充；HAR 热录制已从 1.0 supported contract 移出，`har start|stop` 只保留为防误用的显式失败命令。
