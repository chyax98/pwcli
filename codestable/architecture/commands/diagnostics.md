...
| `trace start|stop|inspect` | `documented` | skill command reference diagnostics 记录 |
| `har start|stop|replay|replay-stop` | `documented` | skill 记录 HAR 边界；experience report 明确 start/stop 不是稳定证据路径 |
| `video start|stop` | `documented` | skill command reference diagnostics 记录 |

**状态分布：** proven 4 / documented 3 / experimental 0
