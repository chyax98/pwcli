...
| `show` | positional runId、`--command <name>`、`--text <text>`、`--limit <n>`、`--since <iso>`、`--fields <fields>` |
...
- action 命令产生的 run events 位于 `.pwcli/runs/<runId>/events.jsonl`，diagnostics 命令只读取和投影。
...
- `har start|stop` 当前只暴露 substrate 边界，稳定证据优先 `network` 和 `diagnostics export`。
...
| 命令 | 状态 | 依据 |
|---|---|---|
| `diagnostics digest/export/bundle/runs/show/grep/timeline` | `proven` | dogfood 覆盖 digest/export/runs/show/grep，test plan 覆盖 bundle/timeline |
| `console` | `proven` | dogfood page error and console 场景覆盖 |
| `network` | `proven` | dogfood 500、request body、route patch network 查询覆盖 |
| `errors` | `proven` | dogfood page error query 覆盖 |
| `trace start|stop|inspect` | `proven` | artdog/tracedog dogfood 覆盖 trace artifact 生成和 inspect；`check:trace-inspect` 固化 CLI 解析 |
| `har start|stop|replay|replay-stop` | `documented` | hardog 验证 start/stop 当前返回 supported=false limitation；replay 仍是边界能力 |
| `video start|stop` | `proven` | artdog dogfood 覆盖 video start/stop 和 WebM artifact 输出 |

**状态分布：** proven 6 / documented 1 / experimental 0
