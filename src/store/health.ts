...
export function compactDoctorDiagnostic(diagnostic: DoctorDiagnostic): DoctorDiagnostic {
  ...
  switch (diagnostic.kind) {
    ...
    case "modal-state":
      return {
        ...diagnostic,
        details: { sessionName: stringValue(d.sessionName), code: stringValue(d.code) },
      };
    case "html-modal":
      return {
        ...diagnostic,
...
> {
  ...
  return browsers.map((browser) => {
    ...
    try {
      ...
      return match
        ? { browser, installed: true, path: resolve(browsersPath, match) }
...
export async function inspectEnvironment(cwd?: string): Promise<DoctorDiagnostic> {
  ...
  const nodeOk = compareNodeVersion(version, minimum) >= 0 && parsed.major < maximumMajorExclusive;
