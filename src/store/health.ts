...
export function compactDoctorDiagnostic(
	...
	switch (diagnostic.kind) {
		...
		case "observe-status": {
			...
			return {
				...
				details: {
					...
					page: {
						url: stringValue(objectRecord(d.page).url),
						title: stringValue(objectRecord(d.page).title),
					},
					workspace: {
		...
		case "state-path":
			return {
				...
				details: {
					...
					...(stringValue(d.parseError)
						? { parseError: stringValue(d.parseError) }
						: {}),
				},
			};
		...
		case "environment": {
			...
			return {
				...diagnostic,
				status: issues.some((i) => i.status === "fail")
					? "fail"
					: issues.length > 0
						? "warn"
						: "ok",
...
export function inspectProfilePath(input?: string): DoctorDiagnostic {
	...
	const writable = exists
		? canWritePath(resolvedPath)
		: canWritePath(resolve(resolvedPath, ".."));
	return {
		kind: "profile-path",
...
export function inspectStatePath(input?: string): DoctorDiagnostic {
	...
	if (!exists)
		return {
			...
			details: { requestedPath: input, resolvedPath, exists },
		};
		...
		return new Promise((resolveResult) => {
			...
			socket.on("error", (error) => {
				...
				finish({
					...
					details: { ...baseDetails, error: error.message },
				});
			});
		});
	}
...
function playwrightChromiumRequirement(): {
	revision?: string;
	browserVersion?: string;
	prefixes: string[];
} {
	const fallback = { prefixes: ["chromium"] };
	try {
		const root = dirname(
			requireFromHere.resolve("playwright-core/package.json"),
		);
		const manifest = JSON.parse(
			readFileSync(resolve(root, "browsers.json"), "utf8"),
		) as {
			browsers?: Array<{
				name?: string;
				revision?: string;
		...
		const headlessShell = manifest.browsers?.find(
			(browser) => browser.name === "chromium-headless-shell",
		);
		const revision = chromium?.revision ?? headlessShell?.revision;
		if (!revision) return fallback;
		return {
			revision,
			browserVersion: chromium?.browserVersion ?? headlessShell?.browserVersion,
			prefixes: [`chromium-${revision}`, `chromium_headless_shell-${revision}`],
		};
	} catch {
		return fallback;
	}
...
> {
	...
	if (!browsersPath) {
		const macosCache = resolve(homedir(), "Library/Caches/ms-playwright");
		if (existsSync(macosCache)) browsersPath = macosCache;
	}

	const requirement = playwrightChromiumRequirement();
	try {
		if (!browsersPath) {
			return [
				{
		...
		const match = requirement.prefixes
			.map((prefix) =>
				entries.find(
			...
			? [
					{
						...
						revision: requirement.revision,
						browserVersion: requirement.browserVersion,
					},
				]
			: [
					{
		...
		return [
			{ browser: "chromium", installed: false, revision: requirement.revision },
		];
	}
}
...
export async function inspectEnvironment(
	...
	const nodeOk =
		compareNodeVersion(version, minimum) >= 0 &&
		parsed.major < maximumMajorExclusive;
