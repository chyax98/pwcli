...
async function readSessionRuntimeConfig(
	configPath?: string,
): Promise<SessionRuntimeConfig> {
...
function sessionRuntimeConfigPath(sessionName: string) {
	return resolve(
		join(".pwcli", "session-config", `${sessionName}.config.json`),
	);
}
...
export async function writeSessionRuntimeConfig(options: {
	...
	if (!options.baseConfigPath && !options.recordHar && !options.recordVideo)
		return undefined;
...
function bootstrapConfigPath(sessionName?: string) {
	return resolve(
		join(".pwcli", "bootstrap", `${sessionName ?? "default"}.json`),
	);
}
...
export async function getSessionDefaults(): Promise<SessionDefaults> {
	try {
		...
		return {
			headed:
				typeof config.headed === "boolean"
					? config.headed
					: DEFAULT_SESSION_DEFAULTS.headed,
			trace:
				typeof config.trace === "boolean"
					? config.trace
					: DEFAULT_SESSION_DEFAULTS.trace,
			diagnosticsRecords:
