...
function printEnvironmentCommandError(
  ...
  if (code === "CLOCK_LIMITATION") {
    printCommandError(command, {
      ...
      message:
        rest.join(":") ||
        "Clock emulation is unavailable on the current managed session substrate.",
      suggestions: [
...
export function registerEnvironmentCommand(program: Command): void {
  ...
  addSessionOption(
    environment.command("offline <mode>").description("Set managed-session network offline mode"),
  ...
  addSessionOption(
    ...
    async (
      ...
      try {
        ...
        const accuracy = options.accuracy === undefined ? undefined : Number(options.accuracy);
  ...
  ).action(async (perm: string[], options: { session?: string }, command: Command) => {
    try {
      const sessionName = requireSessionName(options, command);
      if (perm.length === 0) {
  ...
  addSessionOption(
    permissions.command("clear").description("Clear all BrowserContext permission overrides"),
  ...
  const clock = environment.command("clock").description("Control managed-session clock emulation");
