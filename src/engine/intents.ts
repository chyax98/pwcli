...
const INTENT_DEFINITIONS: Record<IntentName, IntentDefinition> = {
	...
	close_dialog: {
		...
		rules: [
			...
			textRule("Cancel", 88),
			selectorRule(
				"[aria-label='Close'], [data-testid*='close'], .close, .modal-close",
				80,
			),
		],
	},
	...
	back_navigation: {
		...
		rules: [
			...
			selectorRule(
				"a[rel='prev'], button[aria-label='Back']",
				85,
				"prev-selector",
			),
		],
	...
	primary_cta: {
		...
		rules: [
			...
			textRule("Start", 91),
			selectorRule(
				"[data-primary], button.primary, a.primary",
				82,
				"primary-selector",
			),
		],
...
export async function managedFindBest(options: {
	...
	for (const [ruleIndex, rule] of definition.rules.entries()) {
		...
		const ref =
			typeof located.data.ref === "string" ? located.data.ref : undefined;
		const locatedCandidates = Array.isArray(located.data.candidates)
			? located.data.candidates
			: [];
