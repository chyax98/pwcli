...
process.env.PLAYWRIGHT_DAEMON_SESSION_DIR ??= join(
	pwcliRuntimeDir,
	"playwright-daemon",
);
process.env.PLAYWRIGHT_SERVER_REGISTRY ??= join(
	pwcliRuntimeDir,
	"playwright-registry",
);

const sessionModule = require(
	join(playwrightCoreRoot, "lib/tools/cli-client/session.js"),
);
const registryModule = require(
	join(playwrightCoreRoot, "lib/tools/cli-client/registry.js"),
);
const serverRegistryModule = require(
	join(playwrightCoreRoot, "lib/serverRegistry.js"),
);
...
const SESSION_LOCK_TIMEOUT_MS = positiveDurationEnv(
	"PWCLI_SESSION_LOCK_TIMEOUT_MS",
	120_000,
);
const SESSION_LOCK_STALE_MS = positiveDurationEnv(
	"PWCLI_SESSION_LOCK_STALE_MS",
	600_000,
);
...
const SESSION_STARTUP_LOCK_HOLD_MS = positiveDurationEnv(
	"PWCLI_SESSION_STARTUP_LOCK_HOLD_MS",
	0,
);
...
function validateSessionName(name?: string) {
	...
	if (sessionName.length > MAX_SESSION_NAME_LENGTH) {
		throw new Error(
			`SESSION_NAME_TOO_LONG:${sessionName}:${MAX_SESSION_NAME_LENGTH}`,
		);
	}
...
async function withPwcliPlaywrightOutput<T>(
	outputDir: string,
	fn: () => Promise<T>,
) {
...
async function getSessionEntry(sessionName?: string) {
	...
	const entry = registry.entry(clientInfo, resolvedSessionName) as
		| ManagedSessionEntry
		| undefined;
	return {
		clientInfo,
...
async function writeDaemonPidFile(
	...
	await writeFile(daemonPidPath(daemonDir, sessionName), String(pid));
}

async function readDaemonPidFile(
	daemonDir: string,
	sessionName: string,
): Promise<number | null> {
	try {
		const text = await readFile(daemonPidPath(daemonDir, sessionName), "utf8");
...
async function deleteDaemonPidFile(daemonDir: string, sessionName: string) {
	await rm(daemonPidPath(daemonDir, sessionName), { force: true }).catch(
		() => {},
	);
}

/** Find all orphaned playwright-core worker processes (PPID=1). */
function findPlaywrightOrphanPids(): number[] {
	try {
		const output = execSync("ps ax -o pid,ppid,command", { encoding: "utf8" });
		const orphans: number[] = [];
		for (const line of output.split("\n")) {
			if (!line.includes("playwright-core")) continue;
			const parts = line.trim().split(/\s+/);
			const pid = Number(parts[0]);
			const ppid = Number(parts[1]);
			if (ppid === 1 && pid > 0) orphans.push(pid);
		}
		return orphans;
	} catch {
		return [];
...
function forceKillPids(pids: number[]) {
...
// ── Module-level daemon tracking for explicit stop/reset cleanup ──
...
function sessionStartupLockKey(
	...
	while (true) {
		try {
			...
			if (Date.now() - startedAt > options.timeoutMs) {
				throw new Error(
					`SESSION_BUSY:${options.sessionName}:${options.timeoutMs}`,
				);
			}
...
async function stopSessionEntry(
	...
	const pidDirs = [entry.daemonDir, workspaceDaemonDir(workspaceDir)].filter(
	...
	if (await session.canConnect().catch(() => false)) {
		await session
			.run(createClientInfo(), {
				_: [
					"run-code",
					"async page => { await page.context().close(); return 'context-closed'; }",
				],
			})
			.catch(() => {});
	}

	// 2. Try graceful daemon stop via socket
	...
	forceKillPids(findPlaywrightOrphanPids());

	// 5. Delete session config
	if (typeof session.deleteSessionConfig === "function") {
		await session.deleteSessionConfig();
	}

	// 6. Clean up user data dirs and error logs for this session
	const dirEntries = await readdir(entry.daemonDir).catch(() => [] as string[]);
	const leftovers = dirEntries.filter(
		(f) =>
			f.startsWith(`ud-${name}-`) ||
			f === `${name}.err` ||
			f.startsWith(`stale${name}.`),
	);
	await Promise.all(
		leftovers.map((f) =>
			rm(join(entry.daemonDir, f), { recursive: true }).catch(() => {}),
		),
	);
}

export async function getManagedSessionEntry(sessionName?: string) {
	const { sessionName: resolvedSessionName, entry } =
...
export async function listAttachableBrowserServers(): Promise<AttachableBrowserServerList> {
	if (!serverRegistry || typeof serverRegistry.list !== "function") {
		return {
			...
			servers: [],
			limitation:
				"Playwright server registry is not available in this playwright-core build.",
		};
	}

	const clientInfo = createClientInfo();
	const currentWorkspaceDir = clientInfo.workspaceDir ?? process.cwd();
	const entries = (await serverRegistry.list()) as Map<
		string,
		PlaywrightBrowserServerDescriptor[]
	>;
...
async function ensureManagedSessionUnlocked(
	options?: EnsureManagedSessionOptions,
) {
	const { clientInfo, registry, sessionName, entry } = await getSessionEntry(
		options?.sessionName,
	);
	...
	const nextEntry =
		options?.reset || !entry
			? await (async () => {
					const daemonInfo = (await withPwcliPlaywrightOutput(
						resolve(
...
export async function ensureManagedSession(
	...
	const { clientInfo, sessionName } = await getSessionEntry(
		options?.sessionName,
	);
	...
	try {
		const responsePromise = new Promise<unknown>((resolve, reject) => {
			...
			timer = setTimeout(() => {
				...
				const message =
					options.timeoutMessage ?? "managed session command timed out";
				connection.close();
...
export async function stopAllManagedSessions() {
	...
	for (const entry of allEntries) {
		...
		try {
			await stopSessionEntry(entry, createClientInfo().workspaceDir);
	...
	for (const dir of daemonDirs) {
		...
		const leftovers = files.filter(
			(f) =>
				f.endsWith(".err") || f.startsWith("stale-") || f.endsWith(".session"),
		);
...
function findSection(text: string, header: string) {
	...
	return text
		.slice(start + marker.length, next === -1 ? text.length : next)
		.trim();
}
...
export function parseDownloadEvent(text: string) {
	const line = parseEventLines(text).find((entry) =>
		entry.startsWith("- Downloaded file "),
	);
	if (!line) {
		return null;
	}
...
export async function managedOpen(
	...
	if (options?.sessionName && options.enforceActionPolicy !== false) {
		...
		const domains = Array.isArray(allowlist?.data?.result?.allowedDomains)
			? normalizeAllowedDomains(
					allowlist.data.result.allowedDomains as string[],
				)
			: [];
	...
	await managedEnsureDiagnosticsHooks({
		sessionName: options?.sessionName,
	}).catch(() => {});
	return {
		session: {
...
export async function managedBootstrapApply(options: {
	...
	if (options.headersFile) {
		...
		const parsed = JSON.parse(await readFile(headersFile, "utf8")) as Record<
			string,
			unknown
		>;
	...
	const scriptContents = await Promise.all(
		initScripts.map((file) => readFile(file, "utf8")),
	);
    ...
    ${scriptContents
			.map(
				(content) =>
					`await context.addInitScript({ content: ${JSON.stringify(content)} });`,
			)
			.join("\n    ")}
    state.bootstrap = {
      applied: true,
...
export async function managedAllowedDomainsStatus(options?: {
	sessionName?: string;
}) {
	const result = await managedRunCode({
		sessionName: options?.sessionName,
...
export async function managedAllowedDomainsClear(options?: {
	sessionName?: string;
}) {
	const result = await managedRunCode({
		sessionName: options?.sessionName,
...
export async function managedEnsureDiagnosticsHooks(options?: {
	sessionName?: string;
}) {
	const result = await managedRunCode({
		sessionName: options?.sessionName,
      ...
      const keepPageError = (record) => {
        const existingIndex = state.pageErrorRecords.findIndex(existing => {
          if (existing.kind !== 'pageerror' && existing.kind !== 'weberror')
            return false;
          return existing.text === record.text &&
            existing.pageId === record.pageId &&
            existing.navigationId === record.navigationId;
        });
        if (existingIndex !== -1) {
          if (record.kind === 'weberror' && state.pageErrorRecords[existingIndex].kind === 'pageerror')
            state.pageErrorRecords[existingIndex] = { ...state.pageErrorRecords[existingIndex], ...record };
          return;
        }
        keep(state.pageErrorRecords, record);
      };
      ...
      if (!context.__pwcliContextDiagnosticsInstalled) {
        ...
        context.on('weberror', webError => {
          const sourcePage = typeof webError.page === 'function' ? webError.page() : null;
          const error = typeof webError.error === 'function' ? webError.error() : webError;
          const location = typeof webError.location === 'function' ? webError.location() : undefined;
          keepPageError({
            kind: 'weberror',
            sessionName,
            timestamp: now(),
            pageId: sourcePage ? ensurePageId(sourcePage) : null,
            navigationId: sourcePage ? ensureNavigationId(sourcePage) : null,
