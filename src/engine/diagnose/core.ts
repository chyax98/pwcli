...
function isResourceLoadNoise(text: string | null): boolean {
	...
	if (
		...
		/adtrafficquality|googlesyndication|googleadservice|doubleclick|facebook\.com\/tr/.test(
			text,
		)
	)
		return true;
...
function toPageErrorSignal(record: Record<string, unknown>): SignalRecord {
	const text = asString(record.text) ?? "";
	return {
		kind: asString(record.kind) ?? "pageerror",
		timestamp: asString(record.timestamp),
		summary: text,
		details: {
			text,
...
export function buildSessionDigestFromExport(
	...
	const currentPage = asObject(
		data.currentPage ?? exported.page ?? workspace.page,
	);
	...
	const firstPartyFailedRequests = failedRequests.filter(
		(record) =>
			!isTrackingPixel(asString(record.url)) && !isNoisyNetworkFailure(record),
	);
...
export function buildRunDigest(
	...
	const lastTimestamp =
		commandCount > 0 ? eventTimestamp(events.at(-1) ?? {}) : null;
	const sessionName =
		events
			.map((event) => asString(event.sessionName))
	...
	const failureCount = events.filter(
		(event) =>
			Boolean(event.failed) || Object.keys(asObject(event.failure)).length > 0,
	).length;
...
export function buildDiagnosticsAuditConclusion(input: {
	...
	const failedRequestCount =
		asNumber(summary.firstPartyFailedRequestCount) ??
		asNumber(summary.failedRequestCount) ??
		0;
	const consoleErrorCount =
		asNumber(summary.criticalConsoleErrorCount) ??
		asNumber(summary.consoleErrorCount) ??
		0;
	...
	const failedAt = failureLikely
		? (asString(selectedRunEvent?.ts) ??
			...
			null)
		: null;
	const failedCommand = failureLikely
		? (asString(selectedRunEvent?.command) ?? null)
		: null;
	...
	return {
		...
		agentNextSteps: failureLikely
			...
			: [
					"continue planned workflow",
					"verify expected business outcome with assertions",
				],
	};
}
...
export async function managedErrors(
	...
	if (options?.current) {
		...
		const projection = await managedWorkspaceProjection({
...
export async function managedObserveStatus(options?: { sessionName?: string }) {
	...
	const result = await managedRunCode({
      ...
      const har = state.har || {
        ...
        note: 'Session create/recreate records full HAR by default; Playwright writes the HAR when the session closes.',
      };
...
export async function buildDiagnosticsDelta(
	...
	const status = await managedObserveStatus({ sessionName });
	return {
		consoleDelta: Math.max(
			0,
			(status.data.console?.total ?? 0) - before.consoleTotal,
		),
		networkDelta: Math.max(
			0,
			(status.data.network?.total ?? 0) - before.networkTotal,
		),
		pageErrorDelta: Math.max(
			0,
			(status.data.pageErrors?.total ?? 0) - before.pageErrorTotal,
		),
