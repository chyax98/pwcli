...
function boundedOutput(value: string, lineLimit?: number) {
	...
	const lineLimitedValue = lineLimited
	...
	return {
		...
		...(lineLimit && lineLimit > 0
			? { outputLinesShown: Math.min(lines.length, lineLimit) }
			: {}),
...
export async function managedTraceInspect(options: {
	...
	try {
		...
		if (openResult.error) {
			throw new TraceInspectError(
				"TRACE_CLI_FAILED",
				"Playwright trace CLI failed to open trace",
				{
		...
		return {
			data: {
				...
				failed:
					options.failed && options.section === "requests" ? true : undefined,
