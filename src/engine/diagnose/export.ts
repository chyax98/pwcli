...
export async function managedDiagnosticsExport(options?: {
	sessionName?: string;
}) {
	const workspace = await managedObserveStatus({
	...
	const parsed =
		typeof records.data.result === "object" && records.data.result
			? records.data.result
			: {};
		...
		limitTail(
			asArray(value)
				.filter(
					(item) =>
						!timestampField ||
						timestampAtOrAfter(pickFieldPath(item, timestampField), since),
				)
...
export async function writeDiagnosticsExportFile(
	outPath: string,
	data: unknown,
) {
...
export async function readDiagnosticsRunView(options: {
	...
	return {
		...
		events:
			fields.length > 0
				? limited.map((event) => projectRecord(event, fields))
				: limited,
	};
}
...
export async function readDiagnosticsRunDigest(options: {
	...
	return buildRunDigest(
		options.runId,
		await readRunEvents(options.runId),
		options.limit ?? 5,
	);
}
...
async function collectEvidenceArtifacts(events: Record<string, unknown>[]) {
	...
	for (const event of events) {
		...
		push(
			inferArtifactType(asString(event.path) ?? "", command),
			asString(event.path),
		);
	...
	return Promise.all(
		[...unique.entries()].map(([path, type]) => artifactWithSize(type, path)),
	);
}
...
function evidenceSummaryFromBundle(input: {
	...
	const status =
		...
		input.highSignalTimeline.some(
			(entry) => entry.kind === "failure:MODAL_STATE_BLOCKED",
		)
			? "blocked"
...
function renderHandoffReport(input: {
	...
	const nextSteps = Array.isArray(input.auditConclusion.agentNextSteps)
		? input.auditConclusion.agentNextSteps
...
export async function managedDiagnosticsBundle(options: {
	...
	const scopedRunEvents = (
		await Promise.all(
			bundleRuns.map((run) => readRunEvents(run.runId).catch(() => [])),
		)
	).flat();
	const runIds = bundleRuns.map((run) => run.runId);
	const commands = uniqueStrings(
		scopedRunEvents.map((event) => asString(event.command)),
	);
