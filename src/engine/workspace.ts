...
export async function managedAccessibilitySnapshot(options: {
	...
	const yaml = options?.interactiveOnly
		? filterInteractiveAriaYaml(fullYaml)
		: fullYaml;
...
export async function managedWorkspaceProjection(options?: {
	sessionName?: string;
}) {
...
export async function managedPageList(options?: { sessionName?: string }) {
	...
	const current =
		projection.data.workspace.pages.find(
...
export async function managedPageAssess(options?: { sessionName?: string }) {
	...
	const runtimeHintCount = Object.values(signals.runtimeHints ?? {}).filter(
		Boolean,
	).length;
	const hasFrames =
		Number(projection.data.frames.frameCount ?? 0) > 1 ||
		signals.iframeCount > 0;
	const hasDialog =
		Number(projection.data.dialogs.count ?? 0) > 0 ||
		signals.dialogLikeCount > 0;
	...
	const visibleTextDensity =
		signals.visibleTextChars >= 100
			? "high"
			: signals.visibleTextChars >= 40
				? "medium"
				: "low";
	const authSensitivePageLikely =
		signals.passwordFieldCount > 0 ||
		(hasForms &&
			/login|signin|sign-in|auth|account|dashboard|settings/i.test(
				signals.url,
			));
...
function recommendedNextSteps(options: {
	...
	if (options.runtimeProbeRecommended) {
		nextSteps.push(
			`pw code --session ${options.sessionName} '<runtime-probe>'`,
		);
	}
...
async function selectPageById(sessionName: string | undefined, pageId: string) {
	// Use browser-side code with live context.pages() to avoid TOCTOU:
	// getting index from a stale projection then tab-selecting by index
	// risks selecting the wrong tab if pages change between snapshot and action.
	const result = await managedRunCode({
		sessionName,
		source: `async page => {
      ${pageIdRuntimePrelude()}
...
export async function managedTabSelect(options: {
	sessionName?: string;
	pageId: string;
}) {
...
export async function managedTabClose(options: {
	sessionName?: string;
	pageId: string;
}) {
