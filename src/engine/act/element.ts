...
import {
	buildDiagnosticsDelta,
	captureDiagnosticsBaseline,
} from "../diagnose/core.js";
import {
	parseErrorText,
	parsePageSummary,
	parseResultText,
...
export function normalizeSemanticTarget(
...
export function semanticLocatorExpression(
	target: NormalizedSemanticTarget,
): string {
	return target.kind === "role"
		? `page.getByRole(${JSON.stringify(target.role)}, ${JSON.stringify({
...
export function buildRunEvent(
	...
	return {
		...
		navigationId:
			typeof page?.navigationId === "string" ? page.navigationId : null,
		targetKind,
		...details,
	};
}
...
export function parseRefEpochValidation(raw: unknown): RefEpochValidation {
	...
	const obj = (raw !== null && typeof raw === "object" ? raw : {}) as Record<
		string,
		unknown
	>;
...
export function reSnapshotRecovery(
	sessionName: string | undefined,
): ActionFailureRecovery {
	return makeRecovery("re-snapshot", [
...
function isTargetNotFoundError(errorText: string) {
	if (
		...
		/(CLICK|FILL|TYPE|HOVER|CHECK|UNCHECK|SELECT|PRESS)_SEMANTIC_NOT_FOUND/i.test(
			errorText,
		) ||
		/(CLICK|FILL|TYPE|HOVER|CHECK|UNCHECK|SELECT|PRESS)_SELECTOR_NOT_FOUND/i.test(
			errorText,
		)
	) {
		return true;
...
export function throwIfManagedActionError(
	text: string,
...
export function throwManagedActionErrorText(
	...
	if (
		/(CLICK|FILL|TYPE)_SEMANTIC_INDEX_OUT_OF_RANGE|INDEX_OUT_OF_RANGE/i.test(
			errorText,
		)
	) {
	...
	if (
		...
		/not visible|not enabled|not stable|receives pointer events/i.test(
			errorText,
		)
	) {
...
export function semanticClickSource(
	target: NormalizedSemanticTarget,
	button?: string,
) {
...
export function semanticSelectSource(
	target: NormalizedSemanticTarget,
	value: string,
) {
	const nthIndex = target.nth - 1;
...
export function semanticPressSource(
	target: NormalizedSemanticTarget,
	key: string,
) {
	const nthIndex = target.nth - 1;
...
export function isModalBlockedDelta(delta: Record<string, unknown>) {
	return (
		...
		isModalStateBlockedMessage(errorMessage(delta.reason))
	);
}
...
export async function assertFreshRefEpoch(options: {
	sessionName?: string;
	ref: string;
}) {
	...
	try {
		...
		const snapshotText =
			typeof fresh.data?.snapshot === "string" ? fresh.data.snapshot : "";
	...
	throw new ActionFailure({
		...
		details: {
			...
			recovery: {
				...
				previousEpoch: {
					snapshotId:
						(validation as Record<string, unknown>).snapshotId ?? null,
					pageId:
						(validation as Record<string, unknown>).snapshotPageId ?? null,
					navigationId:
						(validation as Record<string, unknown>).snapshotNavigationId ??
						null,
				},
				currentEpoch: {
					pageId: (validation as Record<string, unknown>).currentPageId ?? null,
					navigationId:
						(validation as Record<string, unknown>).currentNavigationId ?? null,
...
export async function dispatchLocatorAction(options: {
	...
	if (command === "click") {
		...
		beforePages =
			(beforeProjection.data.workspace.pages as Array<{ pageId: string }>) ??
			[];
	}
	...
	if (command === "click") {
		const afterProjection = await managedWorkspaceProjection({
			sessionName: resolvedSession,
		});
		const afterPages =
			(afterProjection.data.workspace.pages as Array<{ pageId: string }>) ?? [];
	...
	try {
		...
		await recordFailedActionRun(command, resolvedSession, page, before, error, {
			target: { ref },
		});
		throw error;
	}
...
export async function managedSnapshotStatus(options?: {
	sessionName?: string;
}) {
	const result = await managedRunCode({
		sessionName: options?.sessionName,
...
export async function managedClick(options: {
	...
	const clickOpts = options.button
		? JSON.stringify({ button: options.button })
		: "undefined";
	...
	const locator = options.selector
		? {
				...
				source: selectorActionSource(
					...
					{ selector: options.selector, nth },
					(locatorHandle) =>
						`await ${locatorHandle}.type(${JSON.stringify(options.value)});`,
				),
			}
		: {
				kind: "ref" as const,
