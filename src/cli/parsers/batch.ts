...
export function parseBatchSemanticArgs(
	args: string[],
	commandName: string,
): BatchActionTarget {
	...
	const semanticTargets = [text, role, label, placeholder, testId].filter(
		Boolean,
	).length;
	...
	if (args.includes("--selector") && !selector)
		throw new Error(
			`batch ${commandName} requires a selector after --selector`,
		);
	...
	if (args.includes("--description") && !description)
		throw new Error(
			`batch ${commandName} requires a description after --description`,
		);
	...
	if (args.includes("--nth") && (!Number.isInteger(nth) || (nth ?? 0) < 1)) {
		throw new Error(
			`batch ${commandName} requires a positive integer after --nth`,
		);
	}
	if ((name || description) && !role)
		throw new Error(
			`batch ${commandName} supports --name and --description only with --role`,
		);
	if (nth && !text && !role && !label && !placeholder && !testId) {
		throw new Error(
			`batch ${commandName} supports --nth only with a semantic locator`,
		);
	}

	if (text)
		return {
			semantic: { kind: "text", text, ...(nth ? { nth } : {}) },
			trailingValues,
		};
	if (role) {
		return {
	...
	if (label)
		return {
			semantic: { kind: "label", label, ...(nth ? { nth } : {}) },
			trailingValues,
		};
	if (placeholder) {
		return {
...
export function parseBatchStateTarget(args: string[]): StateTarget | undefined {
	...
	if ((name || description) && !role) {
		throw new Error("batch supports --name and --description only with --role");
	}
