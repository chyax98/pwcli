...
function domainError(rawMessage: string) {
	...
	if (message.startsWith("STATE_TARGET_NOT_FOUND:")) {
		...
		return {
			...
			recovery: {
				...
				commands: ["pw snapshot -i --session <name>"],
			},
	...
	if (message.startsWith("READ_TEXT_SELECTOR_NOT_FOUND:")) {
		return {
			...
			retryable: true,
			suggestions: [
				"Run `pw snapshot -i --session <name>` to inspect current state",
			],
			recovery: {
				kind: "inspect" as const,
				commands: ["pw snapshot -i --session <name>"],
	...
	if (message.startsWith("ACTION_POLICY_DENY:")) {
		const [, category = "action", deniedCommand = "command", policyPath = ""] =
			message.split(":");
		return {
		...
		printCommandError(
			command,
			{
				...
				recovery: error.recovery
					? {
							kind: error.recovery.kind as RecoveryKind,
							commands: error.recovery.commands,
						}
					: undefined,
...
export async function withActionFailureScreenshot<T>(
	...
	try {
		...
		const existingRunDir = isActionFailure(error)
			? (
					(error as ActionFailure).details?.run as
...
async function recordCommandFailure(
	...
	const runDir = existingRunDir ?? (await ensureRunDir(sessionName)).runDir;
	const code = isActionFailure(error)
		? error.code
		: `${command.toUpperCase()}_FAILED`;
	const message = error instanceof Error ? error.message : String(error);
