...
export function buildSemanticTarget(
	args: StateTargetOptions,
	...
	if ((args.name || args.description) && !args.role) {
		throw new Error("--name and --description are only valid with --role");
	}
	if (args.text)
		return { kind: "text", text: args.text, ...(nth ? { nth } : {}) };
	if (args.role) {
		return {
			kind: "role",
			role: args.role,
	...
	if (args.label)
		return { kind: "label", label: args.label, ...(nth ? { nth } : {}) };
	if (args.placeholder) {
		return {
			kind: "placeholder",
...
export function parseStateTarget(options: StateTargetOptions): StateTarget {
	...
	if ((options.name || options.description) && !options.role) {
		throw new Error("--name and --description are only valid with --role");
	}
