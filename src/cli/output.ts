...
function formatDiagnosticsDelta(value: unknown): string[] {
  ...
  if (delta.unavailable) return [`delta unavailable: ${asString(delta.reason) ?? "unknown"}`];
...
function formatVideo(result: CommandResult): string {
  ...
  if (result.data.noVideo === true) lines.push("noVideo=true");
  return lines.join("\n");
}
...
function formatStateTarget(value: unknown): string {
  ...
  if (typeof target.selector === "string") return `selector=${target.selector}${nthSuffix}`;
  if (typeof target.text === "string") return `text=${JSON.stringify(target.text)}${nthSuffix}`;
...
function formatCommandText(command: string, result: CommandResult): string {
  ...
  if (command === "trace start" || command === "trace stop") return formatTrace(result);
  if (command === "video start" || command === "video stop") return formatVideo(result);
...
export function printCommandResult(command: string, result: CommandResult, output?: unknown) {
