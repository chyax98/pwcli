...
type CommandError = {
	...
	retryable?: boolean;
	suggestions?: string[];
	recovery?: { kind: RecoveryKind; commands: string[] };
...
export function outputMode(explicit?: unknown): OutputMode {
	...
	cachedOutputMode = mode === "json" || mode === "text" ? mode : "text";
	return cachedOutputMode;
}
...
function formatAccessibility(result: CommandResult): string {
	const yaml = result.data.snapshot as string | undefined;
	if (!yaml || result.data.empty) return "(empty accessibility tree)";
	return yaml;
}
...
function formatScreenshot(result: CommandResult): string {
	...
	const lines: string[] = [];
	if (path) lines.push(`Screenshot: ${path}`);
...
function formatDiagnosticsRuns(result: CommandResult): string {
	...
	for (const item of runs) {
		...
		lines.push(
			`${asString(run.runId) ?? "-"} session=${asString(run.sessionName) ?? "-"} commands=${asNumber(run.commandCount) ?? 0} failures=${asNumber(summary.failureCount) ?? 0} signals=${asNumber(summary.signalCount) ?? 0} last=${asString(run.lastTimestamp) ?? "-"}`,
		);
	}
	return lines.join("\n");
}
...
function formatTrace(result: CommandResult): string {
	...
	const lines = [
		`trace ${action}${facts.length > 0 ? ` ${facts.join(" ")}` : ""}`,
	];
	...
	if (inspectHint) lines.push(`hint=${inspectHint}`);
	return lines.join("\n");
}
...
function formatSession(result: CommandResult): string {
	...
	const fields = Object.entries(result.data)
		.filter(
			([key]) =>
				key !== "workspace" && key !== "pages" && key !== "currentPage",
		)
		.map(
			([key, value]) =>
				`${key}=${typeof value === "object" ? JSON.stringify(value) : String(value)}`,
		);
	return [page ? `page ${page}` : null, fields.join(" ")]
		.filter(Boolean)
		.join("\n");
}
	...
	if (snapDiff && typeof snapDiff === "object" && "diffText" in snapDiff) {
		...
		lines.push(
			withTextBoundary(
				"accessibility",
				result,
				(snapDiff as { diffText: string }).diffText,
			),
		);
	}
	if (snapDiff && typeof snapDiff === "object" && "diffText" in snapDiff) {
...
function formatVerify(result: CommandResult): string {
	...
	const target = result.data.target
		? ` ${formatStateTarget(result.data.target)}`
		: "";
	const actual =
		"actual" in result.data
			? ` actual=${stringifyValue(result.data.actual)}`
			: "";
	const expected =
		"expected" in result.data
			? ` expected=${stringifyValue(result.data.expected)}`
			: "";
		...
		lines.push(
			[
				...
				candidate.selectorHint
					? `selectorHint=${JSON.stringify(candidate.selectorHint)}`
					: null,
			]
				.filter(Boolean)
				.join(" "),
	...
	for (const item of fields) {
		...
		lines.push(
			[
				...
				field.selectorHint
					? `selectorHint=${JSON.stringify(field.selectorHint)}`
					: null,
			]
				.filter(Boolean)
				.join(" "),
...
function formatCommandText(command: string, result: CommandResult): string {
	...
	if (command === "accessibility") return formatAccessibility(result);
	if (command === "network") return formatNetwork(result);
	if (command === "console") return formatConsole(result);
	if (command === "diagnostics digest") return formatDiagnosticsDigest(result);
	if (command === "diagnostics runs") return formatDiagnosticsRuns(result);
	if (command === "trace inspect") return formatTraceInspect(result);
	if (command === "trace start" || command === "trace stop")
		return formatTrace(result);
	if (command.startsWith("page ")) return formatPage(result);
	if (command.startsWith("session ")) return formatSession(result);
	if (
		[
			"open",
			"click",
	...
	const page = pageLine(result.page);
	return [page ? `page ${page}` : null, stringifyValue(result.data)]
		.filter(Boolean)
		.join("\n");
}
...
export function printCommandResult(
	...
	if (outputMode(output) === "json") {
		printJson(commandEnvelope(command, result));
		return;
	}
	printText(
		withTextBoundary(command, result, formatCommandText(command, result)),
	);
}
