...
import {
	parseBatchSemanticArgs,
	parseBatchStateTarget,
...
function isVerifyAssertion(
	value: string | undefined,
): value is VerifyAssertion {
	return Boolean(
		value && (VERIFY_ASSERTIONS as readonly string[]).includes(value),
	);
}
...
async function readTextMaybeFile(
	...
	if (typeof spec[fileKey] === "string")
		return await readFile(resolve(dir, spec[fileKey]), "utf8");
...
async function loadRouteSpecs(
	...
	const specs = JSON.parse(await readFile(path, "utf8")) as Array<
	...
	for (const spec of specs) {
		...
		const patchJson = await readJsonMaybeFile(
			spec,
			"patchJson",
			"patchJsonFile",
			dir,
		);
		const matchJson = await readJsonMaybeFile(
			spec,
			"matchJson",
			"matchJsonFile",
			dir,
		);
		const headers = await readHeadersMaybeFile(
			spec,
			"headers",
			"headersFile",
			dir,
		...
		const matchQuery = await readHeadersMaybeFile(
			spec,
			"matchQuery",
			"matchQueryFile",
			dir,
		);
		const matchHeaders = await readHeadersMaybeFile(
			spec,
			"matchHeaders",
			"matchHeadersFile",
			dir,
		);
		const patchText = await readHeadersMaybeFile(
			spec,
			"patchText",
			"patchTextFile",
			dir,
		);
		const mergeHeaders = await readHeadersMaybeFile(
			spec,
			"mergeHeaders",
			"mergeHeadersFile",
			dir,
		...
		const result = await managedRoute("add", {
			...
			matchBody:
				typeof spec.matchBody === "string" ? spec.matchBody : undefined,
			matchQuery,
			matchHeaders,
			matchJson,
			patchJson,
			patchText,
			patchStatus:
				spec.patchStatus !== undefined ? Number(spec.patchStatus) : undefined,
			body,
			status: spec.status !== undefined ? Number(spec.status) : undefined,
			contentType:
				typeof spec.contentType === "string" ? spec.contentType : undefined,
			headers,
			mergeHeaders,
			injectHeaders,
	...
	switch (command) {
		...
		case "screenshot": {
			...
			if (args.includes("--selector") && !selector)
				throw new Error(
					`batch step '${rawStep}' requires a selector after --selector`,
				);
		...
		case "page":
			...
			if (args[0] === "list")
				return {
					ok: true,
					command: "page list",
					data: await managedPageList({ sessionName }),
				};
			if (args[0] === "frames")
				return {
					ok: true,
					command: "page frames",
					data: await managedPageFrames({ sessionName }),
				};
			if (args[0] === "dialogs")
				return {
					ok: true,
		...
		case "route":
			...
			if (args[0] === "load") {
				...
				if (!file)
					throw new Error(
						`batch step '${rawStep}' requires a file after route load`,
					);
				const routes = await loadRouteSpecs(file, sessionName, rawStep);
				return {
					ok: true,
					command: "route load",
					data: { loadedCount: routes.length, routes },
				};
			}
			if (args[0] === "remove")
				return {
			...
			if (args[0] === "add") {
				...
				if (!pattern)
					throw new Error(
						`batch step '${rawStep}' requires a pattern after route add`,
					);
				return {
					ok: true,
					command: "route add",
		...
		case "bootstrap": {
			...
			for (let index = 1; index < args.length; index += 1) {
				...
				if (arg === "--init-script") {
					...
					if (!file)
						throw new Error(
							`batch step '${rawStep}' requires a file after --init-script`,
						);
					...
					if (!headersFile)
						throw new Error(
							`batch step '${rawStep}' requires a file after --headers-file`,
						);
					index += 1;
		...
		case "hover": {
			...
			if (!targetRest.semantic && !targetRest.selector && !targetRest.ref) {
				throw new Error(
					`batch ${command} requires a ref, --selector, or semantic locator`,
				);
			}
			const data =
				command === "click"
		...
		case "select": {
			...
			if (targetRest.semantic || targetRest.selector) {
				if (!value)
					throw new Error(
						`batch step '${rawStep}' requires a value after the target`,
					);
				const data =
					command === "fill"
			...
			if (command === "type" && args.length === 1) {
				return {
					ok: true,
					command,
					data: await managedType({ value: args[0], sessionName }),
				};
			}
			if (args.length < 2)
				throw new Error(
					`batch step '${rawStep}' requires ref/target and value`,
				);
			const data =
				command === "fill"
					? await managedFill({
		...
		case "wait": {
			...
			for (let index = 0; index < args.length; index += 1) {
				...
				if (
					...
					if (
						!["visible", "hidden", "stable", "attached", "detached"].includes(
							value,
						)
					) {
			...
			return {
				...
				data: await managedWait({
					...
					networkidle:
						networkidle ||
						(target ? /^network[-_]?idle$/i.test(target) : false),
					sessionName,
				}),
			};
		}
		...
		case "locate": {
			...
			return {
				ok: true,
				command: "locate",
				data: await managedLocate({ sessionName, target }),
			};
		}
		case "get": {
			...
			if (!fact || !["text", "value", "count"].includes(fact))
				throw new Error(
					`batch step '${rawStep}' requires fact: text, value, or count`,
				);
		...
		case "is": {
			...
			if (!state || !["visible", "enabled", "checked"].includes(state))
				throw new Error(
					`batch step '${rawStep}' requires state: visible, enabled, or checked`,
				);
			const target = parseBatchStateTarget(args.slice(1));
			if (!target)
				throw new Error(
					`batch step '${rawStep}' requires a target after state`,
				);
			return {
				ok: true,
				command: `is ${state}`,
		...
		case "verify": {
			...
			let url:
				| { contains?: string; equals?: string; matches?: string }
				| undefined;
			...
			if (data.data.passed === false) {
				...
				(error as Error & { suggestions?: unknown }).suggestions =
...
export async function runBatch(options: {
	...
	for (const [index, argv] of options.commands.entries()) {
		try {
			...
			const message =
				error instanceof Error ? error.message : "batch step failed";
			const reasonCode = extractReasonCode(message);
			const suggestions =
				errorSuggestions(error) ?? buildBatchStepSuggestions(message);
			failedCount += 1;
