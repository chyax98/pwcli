...
function sub(
	name: string,
	fn: (sessionName: string) => Promise<Parameters<typeof print>[1]>,
) {
...
export default defineCommand({
	...
	subCommands: {
		current: sub("current", (sessionName) =>
			managedPageCurrent({ sessionName }),
		),
		list: sub("list", (sessionName) => managedPageList({ sessionName })),
		frames: sub("frames", (sessionName) => managedPageFrames({ sessionName })),
		dialogs: sub("dialogs", (sessionName) =>
			managedPageDialogs({ sessionName }),
		),
