...
const save = defineCommand({
	...
	async run({ args }) {
		...
		try {
			print(
				"profile save-state",
				await managedStateSave(resolveProfileStatePath(name), {
...
const load = defineCommand({
	...
	async run({ args }) {
		...
		try {
			print(
				"profile load-state",
				await managedStateLoad(resolveProfileStatePath(name), {
					sessionName: session(a),
				}),
				a,
			);
	...
	async run({ args }) {
		...
		try {
			print(
				...
				{ data: await removeProfileState(name) },
				a,
			);
		} catch (error) {
			withCliError(
