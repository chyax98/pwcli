...
const info = defineCommand({
	...
	args: {
		...sharedArgs,
		verbose: { type: "boolean", description: "Include provider source" },
	},
	run({ args }) {
		const a = args as CliArgs;
...
const probe = defineCommand({
	...
	args: {
		...
		url: {
			type: "string",
			description: "Protected URL to probe",
			valueHint: "url",
		},
	},
	async run({ args }) {
		...
		try {
			print(
				"auth probe",
				await managedAuthProbe({ sessionName: session(a), url: str(a.url) }),
				a,
			);
		} catch (e) {
	...
	return defineCommand({
		meta: {
			...
			description: provider
				? providerHelpDescription(provider)
				: `Run auth provider ${name}`,
		},
		args: {
			...sharedArgs,
			"save-state": {
			...
			arg: {
				type: "string",
				description:
					"Provider arg key=value. Repeat for targetUrl, phone, smsCode, baseURL, etc.",
				valueHint: "key=value",
			},
			cache: {
				type: "boolean",
				default: true,
				description:
					"Use auth cache (default true). Pass --no-cache to disable.",
			},
		},
		async run({ args }) {
			const a = args as CliArgs;
			try {
				...
				const resolvedArgs = provider.resolveArgs
					? await provider.resolveArgs(rawArgs)
					: rawArgs;
              ...
              return { url, title, isLoginPage: url.includes("/login") || title.includes("登录") || title.toLowerCase().includes("login") };
					...
					const probe = probeResult.data.result as
				...
				if (bool(a.cache) && resolvedArgs.baseURL) {
                  ...
                  return { url, title, isLoginPage: url.includes("/login") || title.includes("登录") || title.toLowerCase().includes("login") };
							...
							const result = navResult.data.result as
								| Record<string, unknown>
								| undefined;
				...
				const result = await managedRunCode({
					sessionName,
					source: buildProviderInvocationSource(
				...
				if (str(a["save-state"]))
					await managedStateSave(str(a["save-state"]), { sessionName });
				const providerResult = result.data.result as
					| Record<string, unknown>
					| undefined;
				print(
					"auth",
					{
						...
						data: {
							...
							...(typeof providerResult?.resolvedBy === "string"
								...
								: {}),
							pageState:
								providerResult?.pageState ?? providerResult?.page ?? null,
							result: providerResult,
							...(str(a["save-state"])
								? { stateSaved: str(a["save-state"]) }
								: {}),
						},
					},
					a,
