...
import { bool, print, printError, session, str, stringArray, withCliError, type CliArgs } from "./_helpers.js";
...
function providerHelpDescription(provider: AuthProviderSpec) {
  ...
  return [
    ...
    args ? `Provider args:\n${args}` : "",
    examples ? `Examples:\n${examples}` : "",
    notes ? `Notes:\n${notes}` : "Notes: auth runs inside an existing session and does not create sessions.",
  ]
    .filter(Boolean)
    .join("\n");
}
...
const info = defineCommand({ meta: { name: "info", description: "Show provider args, examples and notes" }, args: { ...sharedArgs, verbose: { type: "boolean", description: "Include provider source" } }, run({ args }) { const a = args as CliArgs; const name = String((a._ as string[] | undefined)?.[0] ?? ""); const provider = getAuthProvider(name); if (!provider) { printError("auth info", a, { code: "AUTH_PROVIDER_NOT_FOUND", message: `Auth provider '${name}' not found` }); return; } print("auth info", { data: providerInfo(provider, { verbose: bool(a.verbose) }) as unknown as Record<string, unknown> }, a); } });
