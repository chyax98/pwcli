...
async function applySessionDefaults(options: {
	...
	if (!options.traceEnabled)
		return { trace: { requested: false, applied: false } };
	try {
		const trace = await managedTrace("start", {
...
function defaultRecordHarPath(sessionName: string) {
	const stamp = new Date().toISOString().replace(/[:.]/g, "-");
	return resolve(".pwcli", "har", sessionName, `session-${stamp}.har`);
}

function resolveRecordHarConfig(
	a: CliArgs,
	sessionName: string,
): SessionRecordHarConfig | undefined {
	const explicitPath = str(a["record-har"]);
	if (bool(a["no-record-har"])) {
		if (
			explicitPath ||
			str(a["record-har-content"]) ||
			str(a["record-har-mode"]) ||
			str(a["record-har-url-filter"])
		) {
			throw new Error(
				"--no-record-har cannot be combined with --record-har or --record-har-* options",
			);
		}
		return undefined;
	}

	const path = explicitPath ?? defaultRecordHarPath(sessionName);
	return {
		path: resolve(path),
		content:
			optionalChoice(
		...
		...(str(a["record-har-url-filter"])
			? { urlFilter: str(a["record-har-url-filter"]) }
			: {}),
	};
}

async function ensureRecordHarParent(
	recordHar: SessionRecordHarConfig | undefined,
) {
	if (recordHar) await mkdir(dirname(recordHar.path), { recursive: true });
}
...
function parseVideoSize(value: string | undefined) {
	...
	if (!match)
		throw new Error(
			"--record-video-size must use WIDTHxHEIGHT, for example 1280x720",
		);
	...
	if (
		...
		throw new Error(
			"--record-video-size must contain positive integer dimensions",
		);
	}
	return { width, height };
}

function resolveRecordVideoConfig(
	a: CliArgs,
): SessionRecordVideoConfig | undefined {
...
const list = defineCommand({
	...
	args: {
		...
		attachable: {
			type: "boolean",
			description: "Include attachable browser servers",
		},
	},
	async run({ args }) {
		...
		try {
			...
			const enriched = bool(a["with-page"])
				? await Promise.all(
				...
				: sessions;
			const attachable = bool(a.attachable)
				? await listAttachableBrowserServers()
				: undefined;
			print(
				"session list",
				{
			...
			sessionError("session list", a, e, "session list failed");
		}
	},
});
...
const create = defineCommand({
	...
	args: {
		...
		open: {
			type: "string",
			description: "Open URL after creating the named session",
			valueHint: "url",
		},
		profile: {
			type: "string",
			description: "Persistent profile path",
			valueHint: "path",
		},
		"from-system-chrome": {
			type: "boolean",
			description: "Use local Chrome profile",
		},
		"chrome-profile": {
			type: "string",
		...
		persistent: { type: "boolean", description: "Use persistent profile" },
		state: {
			type: "string",
		...
		headed: {
			type: "boolean",
			description: "Open headed browser",
			default: false,
		},
		trace: {
			type: "boolean",
		...
		"init-script": {
			type: "string",
			description: "Init script file",
			valueHint: "path",
		},
		"record-har": {
			type: "string",
			description:
				"Record network activity to a HAR file for this session lifecycle",
			valueHint: "path",
		},
		"no-record-har": {
			type: "boolean",
			description:
				"Disable default full HAR recording for this session lifecycle",
		},
		"record-har-content": {
			type: "string",
		...
		"record-video": {
			type: "string",
			description:
				"Record session video files into a directory for this lifecycle",
			valueHint: "dir",
		},
		"record-video-size": {
			type: "string",
	...
	async run({ args }) {
		...
		try {
			...
			const headed = await resolveHeaded({
				headed: bool(a.headed),
				headless: !bool(a.headed),
			});
			const traceEnabled = await resolveTraceEnabled({
				trace: a.trace as boolean | undefined,
			});
			const systemChrome =
				bool(a["from-system-chrome"]) || str(a["chrome-profile"])
					? await writeChromeProfileConfig(name, str(a["chrome-profile"]))
					: undefined;
			const recordHar = resolveRecordHarConfig(a, name);
			const recordVideo = resolveRecordVideoConfig(a);
			await ensureRecordHarParent(recordHar);
			const runtimeConfig =
			...
			const persistent =
				bool(a.persistent) || Boolean(str(a.profile)) || Boolean(systemChrome);
			...
			if (str(a.state))
				await managedStateLoad(str(a.state) as string, { sessionName: name });
			...
			if (initScripts.length)
				await managedBootstrapApply({ sessionName: name, initScripts });
				...
				"session create",
				{
					...
					data: {
						...
						...(systemChrome
							...
							: {}),
						...(recordHar
							? { recordHar: { ...recordHar, config: configPath } }
							: {}),
						...(recordVideo
							? { recordVideo: { ...recordVideo, config: configPath } }
							: {}),
			...
			sessionError("session create", a, e, "session create failed");
		}
	},
});
...
async function resolveAttachableServer(attachableId: string) {
	...
	if (!server?.canConnect || !server.endpoint)
		throw new Error(
...
const attach = defineCommand({
	...
	async run({ args }) {
		...
		try {
			...
			const target = str(a["attachable-id"])
				? await resolveAttachableServer(str(a["attachable-id"]) as string)
				: await resolveAttachTarget(endpoint, {
						wsEndpoint: str(a["ws-endpoint"]),
						browserUrl: str(a["browser-url"]),
						cdp: str(a.cdp),
					});
			...
			print(
				...
				{
					...result,
					data: { ...result.data, defaults, appliedDefaults, traceEnabled },
				},
				a,
			);
		} catch (e) {
			sessionError("session attach", a, e, "session attach failed");
		}
	},
});
...
const recreate = defineCommand({
	...
	args: {
		...
		"record-har": {
			type: "string",
			description:
				"Record network activity to a HAR file for the recreated session lifecycle",
			valueHint: "path",
		},
		"no-record-har": {
			type: "boolean",
			description:
				"Disable default full HAR recording for this recreated lifecycle",
		},
		"record-har-content": {
			type: "string",
			description: "HAR content policy: omit|embed|attach",
		...
		"record-video": {
			type: "string",
			description:
				"Record session video files into a directory for the recreated lifecycle",
			valueHint: "dir",
		},
		"record-video-size": {
			type: "string",
	...
	async run({ args }) {
		...
		try {
			...
			const currentPage = await getSessionPageSummary(name).catch(
				() => undefined,
			);
			const currentHeaded =
				entry.config.browser?.launchOptions?.headless === false;
			const defaults = await getSessionDefaults();
			...
			if (
				bootstrapConfig &&
				(bootstrapConfig.initScripts.length > 0 || bootstrapConfig.headersFile)
			) {
			...
			print(
				"session recreate",
				{
					session: { scope: "managed", name, default: name === "default" },
					page,
					data: {
						recreated: true,
						headed,
			...
			sessionError("session recreate", a, e, "session recreate failed");
		} finally {
			if (tempDir)
...
const status = defineCommand({
	...
	async run({ args }) {
		const a = args as CliArgs;
		try {
			const name = firstPos(a) as string;
			const status = await getManagedSessionStatus(name);
			if (!status) throw new Error(`SESSION_NOT_FOUND:${name}`);
			print(
				"session status",
				{
					session: {
			...
			sessionError("session status", a, e, "session status failed");
		}
	},
});
const close = defineCommand({
	...
	async run({ args }) {
		...
		try {
			...
			const closeAll = bool(a.all) || name === "all";
			if (closeAll) {
				const result = await stopAllManagedSessions();
				print("session close", { data: { all: true, ...result } }, a);
			} else {
				print(
					"session close",
					{ data: { name, closed: await stopManagedSession(name) } },
					a,
				);
			}
		} catch (e) {
			sessionError("session close", a, e, "session close failed");
		}
	},
});

export default defineCommand({
	meta: { name: "session", description: "Manage named browser sessions" },
	subCommands: { list, create, attach, recreate, status, close },
});
