...
const list = defineCommand({
  ...
  args: {
    ...
    attachable: { type: "boolean", description: "Include attachable browser servers" },
  },
  async run({ args }) {
...
const create = defineCommand({
  ...
  args: {
    ...
    profile: { type: "string", description: "Persistent profile path", valueHint: "path" },
    "from-system-chrome": { type: "boolean", description: "Use local Chrome profile" },
    "chrome-profile": { type: "string", description: "Chrome profile", valueHint: "name" },
    persistent: { type: "boolean", description: "Use persistent profile" },
    state: { type: "string", description: "Load storage state", valueHint: "path" },
  ...
  async run({ args }) {
    ...
    try {
      ...
      if (str(a.profile) && (bool(a["from-system-chrome"]) || str(a["chrome-profile"])))
        throw new Error(
...
async function resolveAttachableServer(attachableId: string) {
  ...
  if (!server?.canConnect || !server.endpoint)
    throw new Error(`attachable server '${attachableId}' not found or not connectable`);
...
const attach = defineCommand({
  ...
  args: {
    ...
    "browser-url": { type: "string", description: "CDP browser URL", valueHint: "url" },
    cdp: { type: "string", description: "CDP port", valueHint: "port" },
    "attachable-id": { type: "string", description: "Attachable server id", valueHint: "id" },
    trace: { type: "boolean", description: "Enable trace recording", default: true },
  },
  async run({ args }) {
    const a = args as CliArgs;
    try {
      ...
      print(
        "session attach",
        { ...result, data: { ...result.data, defaults, appliedDefaults, traceEnabled } },
        a,
      );
    } catch (e) {
  ...
  async run({ args }) {
    ...
    try {
      ...
      print(
        "session status",
        {
          session: { scope: "managed", name: status.name, default: status.name === "default" },
          data: {
            active: status.alive,
  ...
  async run({ args }) {
    ...
    try {
      ...
      if (closeAll) {
        ...
        print("session close", { data: { name, closed: await stopManagedSession(name) } }, a);
      }
    } catch (e) {
