...
const bundle = defineCommand({
	...
	args: {
		...
		task: {
			type: "string",
			description: "Task label for handoff manifest",
			valueHint: "text",
		},
		limit: {
			type: "string",
...
const runs = defineCommand({
	...
	async run({ args }) {
		...
		try {
			...
			print(
				"diagnostics runs",
				{ data: { count: items.length, runs: items } },
				a,
			);
		} catch (e) {
...
export default defineCommand({
	meta: {
		name: "diagnostics",
		description: "Diagnostics export, runs, digest and bundle",
	},
