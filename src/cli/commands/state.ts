...
export default defineCommand({
	...
	args: {
		...
		action: {
			type: "enum",
			options: ["save", "load", "diff"],
			description: "Action",
		},
		before: {
			type: "string",
			description: "Diff before file",
			valueHint: "path",
		},
		after: {
			type: "string",
			description: "Diff after file",
			valueHint: "path",
		},
		"include-values": {
			type: "boolean",
			description: "Include values in diff",
		},
	},
	async run({ args }) {
		...
		try {
			...
			if (action === "save")
				print(
					"state save",
					await managedStateSave(file, { sessionName: session(a) }),
					a,
				);
			else if (action === "load")
				print(
					"state load",
					await managedStateLoad(file as string, { sessionName: session(a) }),
					a,
				);
			else if (action === "diff")
				print(
