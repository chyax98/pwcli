...
const streamServerPath = resolve(
...
export function createStreamStartCommand(options: {
	name: string;
	commandName: string;
	description: string;
	...
	return defineCommand({
		...
		args: {
			...
			port: {
				type: "string",
				description: "Port override (default: random)",
				valueHint: "n",
			},
			"dry-run": {
				type: "boolean",
				description: "Validate command without launching",
			},
		},
		async run({ args }) {
			...
			if (bool(a["dry-run"])) {
				print(
					options.commandName,
					{ data: { sessionName, launchable: true } },
					a,
				);
				return;
			}
			try {
				...
				if (startupError) {
					printError(options.commandName, a, {
				...
				withCliError(options.commandName, a, error, options.failureMessage);
			}
		},
	});
}

export function createStreamStatusCommand(options: {
	name: string;
	commandName: string;
	description: string;
}) {
	return defineCommand({
		...
		async run({ args }) {
			...
			print(
				options.commandName,
				{ data: { ...record, healthy: await isHealthy(record.url) } },
				a,
			);
		},
	});
...
export function createStreamStopCommand(options: {
	name: string;
	commandName: string;
	description: string;
}) {
	return defineCommand({
		...
		async run({ args }) {
			...
			print(
				options.commandName,
				{ data: { stopped: true, sessionName: record.sessionName } },
				a,
			);
		},
	});
...
export const streamStartCommand = createStreamStartCommand({
	name: "start",
	commandName: "stream start",
	failureMessage: "stream start failed",
	description:
...
export const streamStatusCommand = createStreamStatusCommand({
	name: "status",
	commandName: "stream status",
	description:
...
export const streamStopCommand = createStreamStopCommand({
	name: "stop",
