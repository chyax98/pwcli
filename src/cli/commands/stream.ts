...
export function createStreamStartCommand(options: {
  ...
  return defineCommand({
    ...
    async run({ args }) {
      ...
      if (bool(a["dry-run"])) {
        print(options.commandName, { data: { sessionName, launchable: true } }, a);
        return;
      }
      try {
        const child = spawn(
        ...
        for (let i = 0; i < 20; i += 1) {
          ...
          if (record && (await isHealthy(record.url))) {
            print(options.commandName, { data: { started: true, ...record } }, a);
            return;
          }
...
export function createStreamStatusCommand(options: {
  ...
  return defineCommand({
    ...
    async run({ args }) {
      ...
      print(options.commandName, { data: { ...record, healthy: await isHealthy(record.url) } }, a);
    },
  });
}
...
export function createStreamStopCommand(options: {
  ...
  return defineCommand({
    ...
    async run({ args }) {
      ...
      print(options.commandName, { data: { stopped: true, sessionName: record.sessionName } }, a);
    },
  });
}
