...
async function readValues(args: CliArgs) {
	...
	if (!raw) {
		throw new Error(
			"profile save-auth requires --values <json> or --file <path>",
		);
	}
...
const save = defineCommand({
	...
	args: {
		...
		values: {
			type: "string",
			description: "JSON values object",
			valueHint: "json",
		},
		file: {
			type: "string",
			description: "Path to JSON values object",
			valueHint: "path",
		},
	},
	async run({ args }) {
...
const list = defineCommand({
	...
	async run({ args }) {
		...
		try {
			...
			print(
				"profile list-auth",
				{ data: { count: profiles.length, profiles } },
				a,
			);
		} catch (error) {
