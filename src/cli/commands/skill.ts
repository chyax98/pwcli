...
const path = defineCommand({
	...
	run() {
		printSuccess("skill path", {
			path: resolvePackagedSkillRoot(),
			info: getPackagedSkillInfo(),
		});
	},
});
const install = defineCommand({
	...
	run({ args }) {
		...
		try {
			printSuccess(
				"skill install",
				installPackagedSkill(firstPos(a) as string),
			);
		} catch (e) {
	...
	run({ args }) {
		...
		process.stdout.write(
			`${data.references
				.map(
					(item) =>
						`${item.key}\t${item.kind}\t${item.path}${item.exists ? "" : "\tmissing"}`,
				)
				.join("\n")}\n`,
		);
	},
...
const show = defineCommand({
	...
	args: {
		...
		full: {
			type: "boolean",
			description: "Include main skill and all reference files",
		},
	},
	run({ args }) {
		const a = args as CliArgs;
