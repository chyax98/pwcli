...
export default defineCommand({
	...
	args: {
		...
		"auth-provider": {
			type: "string",
			description: "Resolve auth provider",
			valueHint: "name",
		},
		profile: {
			type: "string",
			description: "Inspect profile path",
			valueHint: "path",
		},
		state: {
			type: "string",
			description: "Inspect state path",
			valueHint: "path",
		},
		endpoint: {
			type: "string",
	...
	async run({ args }) {
		...
		try {
			...
			if (str(a.endpoint))
				diagnostics.push(await probeEndpoint(str(a.endpoint)));
			if (str(a["auth-provider"]))
				diagnostics.push({
					kind: "auth-provider",
					status: getAuthProvider(str(a["auth-provider"]) as string)
						? "ok"
						: "fail",
				...
				try {
					...
					const modals =
						statusData.modals && typeof statusData.modals === "object"
							...
							: null;
					const modalCount =
						typeof modals?.count === "number" ? modals.count : 0;
					if (modalCount > 0) {
						diagnostics.push({
							kind: "html-modal",
							status: "warn",
							...
							details: {
								sessionName,
								count: modalCount,
								items: Array.isArray(modals?.items)
									? modals.items.slice(0, 3)
									: [],
							},
						});
					}
			...
			print(
				"doctor",
				{
					data: {
						...
						diagnostics: bool(a.verbose)
							? diagnostics
							: diagnostics.map(compactDoctorDiagnostic),
