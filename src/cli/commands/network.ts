...
export default defineCommand({
  ...
  args: {
    ...
    "resource-type": { type: "string", description: "Resource type", valueHint: "type" },
    text: { type: "string", description: "Text filter", valueHint: "text" },
    url: { type: "string", description: "URL filter", valueHint: "url" },
    kind: {
      type: "enum",
      options: ["request", "response", "requestfailed", "console-resource-error"],
    ...
    "include-body": { type: "boolean", description: "Include captured bodies" },
    ...
    try {
      print(
        ...
        await managedNetwork({
          ...
          requestId: str(a["request-id"]),
          method: str(a.method),
          status: str(a.status),
          resourceType: str(a["resource-type"]),
          text: str(a.text),
          url: str(a.url),
          kind: str(a.kind) as
            | "request"
          ...
          includeBody: bool(a["include-body"]),
          limit: num(a.limit),
        }),
        a,
      );
    } catch (e) {
      withCliError("network", a, e);
    }
  },
});
