...
const clockSet = defineCommand({ meta: { name: "set", description: "Set fake clock time" }, args: sharedArgs, async run({ args }) { const a = args as CliArgs; try { print("environment clock set", await managedEnvironmentClockSet(new Date(firstPos(a) as string).toISOString(), { sessionName: session(a) }), a); } catch (e) { withCliError("environment clock set", a, e); } } });
const clockResume = defineCommand({ meta: { name: "resume", description: "Resume clock" }, args: sharedArgs, async run({ args }) { const a = args as CliArgs; try { print("environment clock resume", await managedEnvironmentClockResume({ sessionName: session(a) }), a); } catch (e) { withCliError("environment clock resume", a, e); } } });

export default defineCommand({ meta: { name: "environment", description: "Environment and clock controls" }, subCommands: { offline, geolocation: defineCommand({ meta: { name: "geolocation", description: "Geolocation controls" }, subCommands: { set: geoSet } }), permissions: defineCommand({ meta: { name: "permissions", description: "Permission controls" }, subCommands: { grant: permGrant, clear: permClear } }), clock: defineCommand({ meta: { name: "clock", description: "Clock controls" }, subCommands: { install: clockInstall, set: clockSet, resume: clockResume } }) } });
