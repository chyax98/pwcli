...
const offline = defineCommand({
	...
	async run({ args }) {
		...
		try {
			...
			print(
				"environment offline",
				await managedEnvironmentOffline(firstPos(a) as "on" | "off", {
	...
	async run({ args }) {
		...
		try {
			...
			if (latitude === undefined || longitude === undefined) {
				throw new Error(
					"environment geolocation set requires --lat <lat> --lng <lng>",
				);
			}
			print(
				"environment geolocation set",
	...
	async run({ args }) {
		...
		try {
			...
			print(
				...
				await managedEnvironmentClockSet(
					new Date(firstPos(a) as string).toISOString(),
					{
						sessionName: session(a),
					},
				),
	...
	subCommands: {
		...
		"allowed-domains": defineCommand({
			meta: {
				...
				description: "Allowed navigation domains",
			},
			subCommands: {
				set: domainsSet,
