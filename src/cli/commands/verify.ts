...
export default defineCommand({
	...
	args: {
		...
		assertion: {
			...
			options: assertions,
			description:
				"Assertion: text, text-absent, url, visibility, enabled, checked, or count",
		},
		contains: {
			type: "string",
		...
		equals: {
			type: "string",
			description: "Expected exact value",
			valueHint: "value",
		},
		matches: {
			type: "string",
			description: "Expected regex",
			valueHint: "regex",
		},
	...
	async run({ args }) {
		...
		try {
			...
			const url =
				assertion === "url"
					? {
							contains: str(a.contains),
							equals: str(a.equals),
							matches: str(a.matches),
						}
					: undefined;
			const count =
				assertion === "count"
