  ...
  switch (command) {
    ...
    case 'code': {
      ...
      return {
        ok: true,
        command: 'code',
        data: await managedRunCode({ source }),
      };
    }
    case 'snapshot':
      return {
    ...
    case 'page':
      if (args[0] === 'current') {
        return { ok: true, command: 'page current', data: await managedPageCurrent() };
      }
      if (args[0] === 'list') {
        return { ok: true, command: 'page list', data: await managedPageList() };
      }
      if (args[0] === 'frames') {
        return { ok: true, command: 'page frames', data: await managedPageFrames() };
      }
    ...
    case 'click':
      ...
      return {
        ok: true,
        command: 'click',
        data: await managedClick({ ref: args[0] }),
      };
    case 'fill':
      if (args.length < 2) {
    ...
    case 'press':
      ...
      return {
        ok: true,
        command: 'press',
        data: await managedPress(args[0]),
      };
    case 'scroll':
      if (!args.length) {
    ...
    case 'state':
      if (args[0] === 'save') {
        return {
          ok: true,
          command: 'state save',
          data: await managedStateSave(args[1]),
        };
      }
      if (args[0] === 'load' && args[1]) {
        return {
          ok: true,
          command: 'state load',
          data: await managedStateLoad(args[1]),
        };
      }
...
export async function runBatch(options: {
  ...
  for (const rawStep of options.steps) {
    try {
      results.push(await executeBatchStep(rawStep));
    } catch (error) {
      results.push({
        ok: false,
        command: rawStep,
