...
type DcAuthPage = {
	...
	route(
		pattern: string,
		handler: (route: DcAuthRoute) => Promise<void>,
	): Promise<void>;
...
const dcProviderSource = String(
	async (page: DcAuthPage, args: Record<string, string | undefined>) => {
		...
		try {
			authResult = await page.evaluate(
				async ({
					...
					const scope =
						authorizeUrl.searchParams.get("scope") || "public_profile";
					...
					const phoneResp = await fetch(
						`/api/phone/login?X-UA=${encodeURIComponent(baseXua)}`,
						{
							method: "POST",
							headers: {
			...
			if (
				...
				!fallbackUrl.includes("authorize")
			) {
				return {
					ok: true,
					resolvedTargetUrl: targetUrl,
		...
		const redirectUri = authResult.redirectUri.replace(
			/^http:\/\//,
			"https://",
		);
...
export const dcAuthProvider: AuthProviderSpec = {
	...
	description:
		"在现有 session 内执行 DC 登录链。provider 参数统一使用 --arg key=value。",
	...
	args: [
		...
		},
		{
			name: "baseURL",
			description:
				"DC 基础域名。通常不需要传，优先从当前页面或 targetUrl 推导。",
		},
		{
			name: "appPath",
...
async function resolveDcArgs(
	...
	if (providerArgs.instance) {
		throw new Error(
			"dc auth no longer accepts --arg instance. Pass --arg targetUrl=<url>.",
		);
	}
...
function getLocalIp() {
	...
	const preferred = candidates.find(
		(candidate) =>
			/^(?:en|eth|wlan)/.test(candidate.name) &&
