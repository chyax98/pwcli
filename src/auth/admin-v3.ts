...
const adminV3ProviderSource = String(
	async (page: AdminV3AuthPage, args: Record<string, string | undefined>) => {
		...
		const baseURL = String(args.baseURL ?? DEFAULT_BASE_URL)
			...
			.replace(/\/$/, "");
		const targetUrl =
			String(args.targetUrl ?? "").trim() || `${baseURL}${DEFAULT_TARGET_PATH}`;
		...
		if (!baseURL || !targetUrl) {
			throw new Error(
				"ADMIN_V3_AUTH_URL_REQUIRED: admin-v3 auth requires baseURL and targetUrl",
			);
		}
		...
		if (!phoneResult?.ok) {
			throw new Error(
				"ADMIN_V3_PHONE_LOGIN_FAILED: phone login did not return ok",
			);
		}
		...
		if (finalUrl.includes("/login")) {
			throw new Error(
				`ADMIN_V3_AUTH_FAILED: still on login page after full flow. url=${finalUrl}`,
			);
		}
