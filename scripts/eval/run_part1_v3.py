...
# TC-001
c, o, e = run_pw(["session", "create", "eval-p1-1", "--open", BASE_URL], timeout=20)
t = o+e
...
# TC-002
c, o, e = run_pw(["session", "create", "eval-p1-2", "--headed", "--open", BASE_URL], timeout=20)
t = o+e
...
# TC-003
c, o, e = run_pw(["session", "create", "eval-p1-3", "--open", f"{BASE_URL}/login"], timeout=20)
t = o+e
...
# TC-008
c, o, e = run_pw(["session", "recreate", "eval-p1-1", "--open", f"{BASE_URL}/login"], timeout=20)
t = o+e
...
# TC-030
run_pw(["open", f"{BASE_URL}/dashboard", "--session", "eval-p1-1"])
c, o, e = run_pw(["wait", "network-idle", "--session", "eval-p1-1"], timeout=20)
t = o+e
...
# TC-031
c, o, e = run_pw(["wait", "--selector", '[data-testid="stat-users"]', "--session", "eval-p1-1"], timeout=20)
t = o+e
if c==0 and "timeout" not in t.lower():
...
# TC-032
c, o, e = run_pw(["wait", "--text", "Total Users", "--session", "eval-p1-1"], timeout=20)
t = o+e
if c==0 and "timeout" not in t.lower():
...
c, o, e = run_pw(["wait", "--networkidle", "--session", "eval-p1-1"], timeout=20)
t = o+e
if c==0 and "timeout" not in t.lower():
...
# TC-056
run_pw(["open", f"{BASE_URL}/dashboard", "--session", "eval-p1-1"])
proc = subprocess.run("echo '[[\"observe\", \"status\"]]' | node dist/cli.js batch --session eval-p1-1 --stdin-json",
    shell=True, capture_output=True, text=True, timeout=20)
c, t = proc.returncode, proc.stdout + proc.stderr
...
# TC-057
run_pw(["session", "create", "eval-p1-b1", "--open", f"{BASE_URL}/login"], timeout=20)
proc = subprocess.run(
...
domain_map = {f"TC-{i:03d}": f"Domain {(i-1)//10 + 1}" for i in range(1, 73)}
# Override for actual domain boundaries
domain_map.update({
