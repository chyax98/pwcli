...
1. 新页面先 `status`，再 `read-text` / `snapshot -i`。
2. 多 tab 先 `page list` 找 `pageId`。
3. tab 写操作只用 `pageId`，不要用 index/title/URL substring。
4. dialog 先看 `page dialogs` / `status`，再 `dialog accept|dismiss`。
...
| 用 `status` 替代 diagnostics | bug 细查用 diagnostics |

## 恢复路径

- 页面状态不明：`status -s <name>`。
- 多页面混乱：`page list -s <name>`，按 `pageId` select。
- browser dialog 阻塞：action 返回 `modalPending=true` 或 `MODAL_STATE_BLOCKED` 后直接 `dialog accept|dismiss`；不要继续堆叠 `page dialogs`。
- `page dialogs`：仅在未阻塞时读取 observed dialog events，不是 pending browser dialog live list。
- 页面级 modal：结合 `status` 的 modal summary 和 `read-text` 判断；必要时 `doctor` / recreate。
