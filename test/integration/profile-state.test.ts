...
test("profile state save/load/list/remove works across sessions", async () => {
	...
	try {
		...
		const list = await runPw(["profile", "list-state", "--output", "json"], {
			cwd: workspaceDir,
		});
		...
		assert.equal(
			createSecond.code,
			0,
			`second session create failed: ${createSecond.stderr}`,
		);
		...
		const remove = await runPw(
			["profile", "remove-state", profileName, "--output", "json"],
			{
				cwd: workspaceDir,
			},
		);
		assert.equal(
			remove.code,
			0,
			`profile remove-state failed: ${remove.stderr}`,
		);
	} finally {
