...
const server = createServer((_request, response) => {
	...
	response.end(
		`<!doctype html><title>State Diff Fixture</title><main>state-diff</main>`,
	);
});
...
try {
	...
	assert.equal(
		baselineResult.code,
		0,
		`baseline diff failed: ${JSON.stringify(baselineResult)}`,
	);
	...
	assert.equal(
		mutateResult.code,
		0,
		`mutation failed: ${JSON.stringify(mutateResult)}`,
	);
	...
	assert.equal(
		diffResult.code,
		0,
		`state diff failed: ${JSON.stringify(diffResult)}`,
	);
	...
	assert.ok(
		diffEnvelope.data.summary.changedBuckets.includes("sessionStorage"),
	);
	...
	assert.ok(
		diffEnvelope.data.cookies.added.some(
	...
	assert.equal(
		valueDiffResult.code,
		0,
		`value diff failed: ${JSON.stringify(valueDiffResult)}`,
	);
	...
	assert.ok(
		valueDiffEnvelope.data.summary.changedBuckets.includes("localStorage"),
	);
	assert.ok(
		valueDiffEnvelope.data.summary.changedBuckets.includes("sessionStorage"),
	);
	assert.ok(
		valueDiffEnvelope.data.cookies.changed.some(
			(entry) =>
				entry.name === "session_id" && entry.changedFields.includes("value"),
		),
	);
	assert.ok(
	...
	const savedBaseline = JSON.parse(await readFile(beforePath, "utf8")) as {
		version: number;
	};
	const savedAfter = JSON.parse(await readFile(afterPath, "utf8")) as {
		version: number;
	};
