...
describe("fixture server", async () => {
	...
	it("POST /login with correct credentials returns 302 and Set-Cookie", async () => {
		...
		assert.ok(
			setCookie.includes("session=demo-session"),
	...
	it("GET /dashboard without cookie returns 302 to /login", async () => {
		const res = await fetch("http://localhost:7778/dashboard", {
			redirect: "manual",
		});
		assert.equal(res.status, 302);
...
describe("isThirdPartyUrl", () => {
	it("returns false for same-origin URLs", () => {
		assert.equal(
			isThirdPartyUrl("https://example.com/path", "https://example.com"),
			false,
		);
		assert.equal(
			isThirdPartyUrl("https://example.com:443/path", "https://example.com"),
			false,
		);
	});
	...
	it("returns true for different-origin URLs", () => {
		assert.equal(
			isThirdPartyUrl("https://other.com/path", "https://example.com"),
			true,
		);
		assert.equal(
			isThirdPartyUrl("https://sub.other.com/path", "https://example.com"),
			true,
		);
	});
...
describe("SUPPORTED_BATCH_TOP_LEVEL", () => {
	it("contains all expected batch commands", () => {
		...
		for (const cmd of expected) {
			assert.ok(
				SUPPORTED_BATCH_TOP_LEVEL.includes(
					cmd as (typeof SUPPORTED_BATCH_TOP_LEVEL)[number],
				),
