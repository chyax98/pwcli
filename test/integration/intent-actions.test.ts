...
try {
	...
	const seed = await runPw(
		["code", page, "--session", sessionName, "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	assert.equal(
		findSubmit.code,
		0,
		`find-best submit_form failed: ${findSubmit.stderr}`,
	);
	const submitPayload = findSubmit.json as {
		ok: boolean;
		data: {
			count: number;
			best: { ref?: string; text?: string; strategy?: string } | null;
		};
	};
	...
	assert.equal(
		actCookies.code,
		0,
		`act accept_cookies failed: ${actCookies.stderr}`,
	);
	...
	assert.equal(
		cookieState.code,
		0,
		`cookie status read failed: ${cookieState.stderr}`,
	);
	const cookieText = cookieState.json as {
		ok: boolean;
		data: { text: string };
	};
	...
	assert.equal(
		actSubmit.code,
		0,
		`act submit_form failed: ${actSubmit.stderr}`,
	);
	...
	assert.equal(
		submitState.code,
		0,
		`submit status read failed: ${submitState.stderr}`,
	);
	const submitText = submitState.json as {
		ok: boolean;
		data: { text: string };
	};
