...
await writeFile(
	mergeHeadersPath,
	JSON.stringify({ "x-from-test": "yes" }, null, 2),
	"utf8",
);
await writeFile(
	matchQueryPath,
	JSON.stringify({ tenant: "alpha" }, null, 2),
	"utf8",
);
await writeFile(
	matchHeadersPath,
	JSON.stringify({ "x-tenant": "alpha" }, null, 2),
	"utf8",
);
await writeFile(
	matchJsonPath,
...
await writeFile(
	patchTextPath,
	JSON.stringify({ fallback: "mocked" }, null, 2),
	"utf8",
);
...
try {
	...
	const listResult = await runPw(
		["route", "list", "--session", sessionName, "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	assert.deepEqual(routeList.data.routes[0]?.matchHeaders, {
		"x-tenant": "alpha",
	});
	assert.deepEqual(routeList.data.routes[0]?.matchJson, { tenant: "alpha" });
	assert.deepEqual(routeList.data.routes[0]?.mergeHeaders, {
		"x-from-test": "yes",
	});
	...
	assert.equal(
		clickResult.code,
		0,
		`click failed: ${clickResult.stderr}\n${clickResult.stdout}`,
	);
	...
	assert.equal(
		waitResult.code,
		0,
		`wait failed: ${waitResult.stderr}\n${waitResult.stdout}`,
	);

	const statusResult = await runPw(
		["session", "status", sessionName, "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	assert.equal(
		(statusResult.json as { data: { active: boolean } }).data.active,
		true,
	);
