...
try {
	...
	assert.equal(
		authenticatedEnvelope.data.capability.capability,
		"auth-state-probe",
	);
	...
	assert.equal(
		anonymousProbe.code,
		0,
		`anonymous probe failed: ${JSON.stringify(anonymousProbe)}`,
	);
	...
	assert.equal(
		challengeProbe.code,
		0,
		`challenge probe failed: ${JSON.stringify(challengeProbe)}`,
	);
