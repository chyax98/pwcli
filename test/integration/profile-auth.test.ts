...
test("profile auth save/login/list/remove works with encrypted local profiles", async () => {
	...
	try {
		...
		const listPayload = list.json as {
			ok: boolean;
			data: { profiles: Array<{ name: string }> };
		};
		assert.equal(listPayload.ok, true);
		assert.ok(
			listPayload.data.profiles.some((item) => item.name === profileName),
		);
		...
		const page = await runPw(
			["page", "current", "--session", sessionName, "--output", "json"],
			{
				cwd: workspaceDir,
				env,
			},
		...
		const pagePayload = page.json as {
			ok: boolean;
			data: { currentPage: { url: string } };
		};
		assert.equal(pagePayload.ok, true);
		assert.equal(
			pagePayload.data.currentPage.url,
			"http://localhost:7778/dashboard",
		);

		const remove = await runPw(
			["profile", "remove-auth", profileName, "--output", "json"],
			{
				cwd: workspaceDir,
				env,
			},
		...
		assert.equal(
			remove.code,
			0,
			`profile remove-auth failed: ${remove.stderr}`,
		);
	} finally {
