...
try {
	...
	const listResult = await runPw(
		["session", "list", "--attachable", "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	assert.ok(
		attachableId,
		"expected a connectable attachable server for the source session",
	);
	...
	assert.equal(
		attachResult.code,
		0,
		`session attach failed: ${attachResult.stderr}`,
	);
	...
	const statusResult = await runPw(
		["session", "status", attachedSessionName, "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
