...
try {
	...
	const status = await runPw(
		["stream", "status", "--session", sessionName, "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	const observe = await runPw(
		["status", "--session", sessionName, "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	const statusResponse = await fetch(
		new URL("/status.json", startPayload.data.url),
	);
	...
	assert.ok(
		statusJson.recentEvents?.some((item) => item.command === "screenshot"),
	);
	const frameResponse = await fetch(
		new URL("/frame.jpg", startPayload.data.url),
	);
