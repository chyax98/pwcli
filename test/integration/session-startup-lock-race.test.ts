...
try {
	...
	const failures = results.filter(
		(
			result,
		): result is CliResult & { json: { ok: false; error: { code: string } } } =>
			result.code !== 0 &&
			!!result.json &&
			typeof result.json === "object" &&
	...
	assert.equal(
		failures.length,
		1,
		`expected exactly one failure, got ${JSON.stringify(results)}`,
	);
	assert.equal(
	...
	assert.ok(
		!failures[0].stdout.includes("EADDRINUSE") &&
			!failures[0].stderr.includes("EADDRINUSE"),
