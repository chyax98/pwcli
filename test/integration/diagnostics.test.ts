...
describe("diagnostics", { concurrency: false }, () => {
	...
	it("network shows request records", async () => {
		...
		assert.equal(json.ok, true);
		assert.ok(
			json.data.summary.total > 0,
			"expected at least one network record",
		);
	...
	it("errors recent includes context weberror records", async () => {
		const name = makeSessionName();
		sessionsToClean.push(name);
		...
		await runPw([
			...
			"--session",
			name,
			"async page => { await page.setContent(`<script>setTimeout(() => { throw new Error('pwcli weberror marker') }, 0)</script>`); await page.waitForTimeout(200); }",
			"--output",
			"json",
		]);
		...
		assert.equal(result.code, 0, `errors recent failed: ${result.stderr}`);
		const json = result.json as {
			ok: boolean;
			data: {
				errors: Array<{ kind: string; text: string; location?: unknown }>;
			};
		};
		assert.equal(json.ok, true);
		const webError = json.data.errors.find(
			(error) => error.kind === "weberror",
		);
		assert.ok(webError, "expected weberror record");
		assert.equal(webError.text, "pwcli weberror marker");
		assert.equal(
			json.data.errors.some(
				(error) =>
					error.kind === "pageerror" && error.text === "pwcli weberror marker",
			),
			false,
			"weberror should not be duplicated as pageerror",
		);
	});
	...
	it("doctor --session returns health info", async () => {
		...
		const sessionDiag = json.data.diagnostics.find(
			(d) => d.kind === "observe-status",
		);
