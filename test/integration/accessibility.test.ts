...
describe("accessibility", { concurrency: false }, () => {
	...
	it("basic accessibility returns ARIA YAML", async () => {
		...
		assert.ok(
			json.data.snapshot.includes('button "Click me"'),
			"snapshot should contain button",
		);
		assert.ok(
			json.data.snapshot.includes('link "Link"'),
	...
	it("--interactive-only returns only interactive nodes", async () => {
		...
		assert.equal(
			result.code,
			0,
			`accessibility --interactive-only failed: ${result.stderr}`,
		);
		const json = result.json as {
			ok: boolean;
			data: { format: string; empty: boolean; snapshot: string };
	...
	it("empty page returns empty snapshot", async () => {
		...
		assert.equal(
			result.code,
			0,
			`accessibility empty page failed: ${result.stderr}`,
		);
		const json = result.json as {
			ok: boolean;
			data: { format: string; empty: boolean; snapshot: string };
