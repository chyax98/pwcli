...
try {
	...
	const analyze = await runPw(
		["analyze-form", "--session", sessionName, "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	assert.ok(
		analyzePayload.data.fields.some((field) =>
			field.label?.includes("Username"),
		),
	);
	assert.ok(
		analyzePayload.data.fields.some((field) => field.inputType === "password"),
	);
