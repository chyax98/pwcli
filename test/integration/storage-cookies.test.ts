...
describe("storage and cookies", { concurrency: false }, () => {
	...
	it("reads and mutates localStorage, sessionStorage, and cookies on an explicit origin", async () => {
		const server = createServer((_request, response) => {
			...
			response.end(
				"<!doctype html><title>Storage Cookie Fixture</title><main>storage</main>",
			);
		});
		...
		try {
			...
			assert.equal(
				localSet.code,
				0,
				`storage local set failed: ${localSet.stderr}`,
			);
			...
			assert.equal(
				localRead.code,
				0,
				`storage local read failed: ${localRead.stderr}`,
			);
			...
			assert.equal(
				sessionSet.code,
				0,
				`storage session set failed: ${sessionSet.stderr}`,
			);
			...
			assert.equal(
				sessionRead.code,
				0,
				`storage session read failed: ${sessionRead.stderr}`,
			);
			...
			assert.equal(
				localDelete.code,
				0,
				`storage local delete failed: ${localDelete.stderr}`,
			);
			...
			assert.equal(
				cookieSet.code,
				0,
				`cookies set failed: ${cookieSet.stderr}`,
			);
			...
			assert.equal(
				cookieList.code,
				0,
				`cookies list failed: ${cookieList.stderr}`,
			);
			...
			assert.ok(
				cookieListJson.data.cookies.some(
					(cookie) =>
						cookie.name === "pwcli_cookie" && cookie.value === "cookie-value",
				),
			);
			...
			assert.equal(
				cookieDelete.code,
				0,
				`cookies delete failed: ${cookieDelete.stderr}`,
			);
