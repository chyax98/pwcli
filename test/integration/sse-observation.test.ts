...
const cliEnv = {
	...
	PLAYWRIGHT_DAEMON_SESSION_DIR: resolve(
		workspaceDir,
		".pwcli",
		"playwright-daemon",
	),
	PLAYWRIGHT_SERVER_REGISTRY: resolve(
		workspaceDir,
		".pwcli",
		"playwright-registry",
	),
...
try {
	...
	const sseResult = await runPw(
		["sse", "--session", sessionName, "--output", "json"],
		{
			cwd: workspaceDir,
			env: cliEnv,
		},
	...
	assert.ok(
		sseEnvelope.data.count >= 1,
		`expected SSE records: ${JSON.stringify(sseEnvelope)}`,
	);
