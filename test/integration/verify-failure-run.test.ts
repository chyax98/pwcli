...
try {
	...
	assert.equal(
		create.code,
		0,
		`session create failed: ${JSON.stringify(create)}`,
	);
	...
	assert.equal(
		bundle.code,
		0,
		`diagnostics bundle failed: ${JSON.stringify(bundle)}`,
	);
	...
	const manifest = JSON.parse(
		await readFile(resolve(bundleDir, "manifest.json"), "utf8"),
	) as {
		...
		latestRunEvents?: {
			events?: Array<{ command?: string; failure?: { code?: string } }>;
		};
	};
	...
	assert.ok(
		manifest.latestRunEvents?.events?.some(
			(event) =>
				event.command === "verify" && event.failure?.code === "VERIFY_FAILED",
		),
	...
	assert.ok(
		manifest.summary?.highSignalFindings?.includes("verify text failed"),
	);
