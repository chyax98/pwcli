...
import {
	startFixtureServer,
	stopFixtureServer,
} from "../fixtures/servers/realistic-app.mjs";
import {
	createWorkspace,
	removeWorkspace,
	runPw,
...
try {
	...
	const initial = await runPw(
		["control-state", "--session", sessionName, "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	const initialPayload = initial.json as {
		ok: boolean;
		data: { state: string; actor: string };
	};
	...
	const status = await runPw(
		["stream", "start", "--session", sessionName, "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	const blockedOpenPayload = blockedOpen.json as {
		...
		error: {
			code: string;
			details?: { command?: string; actor?: string | null };
		};
	};
	...
	async function assertHumanControlled(args: string[], command: string) {
		const blocked = await runPw(
			[...args, "--session", sessionName, "--output", "json"],
			{
				cwd: workspaceDir,
			},
		);
		assert.equal(
			blocked.code,
			1,
			`${command} should be blocked while human-controlled`,
		);
	...
	const release = await runPw(
		["release-control", "--session", sessionName, "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	assert.equal(
		resumedOpen.code,
		0,
		`open after release failed: ${resumedOpen.stderr}`,
	);
} finally {
	await runPw(
		["stream", "stop", "--session", sessionName, "--output", "json"],
		{
			cwd: workspaceDir,
		},
	).catch(() => undefined);
