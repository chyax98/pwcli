...
try {
	...
	const visible = await runPw(
		["check-injection", "--session", sessionName, "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	const visiblePayload = visible.json as {
		...
		data: {
			count: number;
			risky: boolean;
			findings: Array<{ pattern: string; visible: boolean }>;
		};
	};
	...
	assert.ok(
		visiblePayload.data.findings.some(
			(item) => item.pattern === "system_prompt",
		),
	);
	assert.ok(
		visiblePayload.data.findings.every((item) => item.visible === true),
	);
	...
	assert.equal(
		hidden.code,
		0,
		`check-injection --include-hidden failed: ${hidden.stderr}`,
	);
