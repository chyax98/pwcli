...
describe("session lifecycle", { concurrency: false }, () => {
	...
	it("open does not create a missing session", async () => {
		...
		assert.equal(
			openResult.code,
			1,
			`open should fail for missing session: ${openResult.stdout}`,
		);
		const openJson = openResult.json as {
			ok: boolean;
			error?: { code: string };
		};
		...
		assert.equal(
			statusResult.code,
			1,
			"open should not create the missing session",
		);
		const statusJson = statusResult.json as {
			ok: boolean;
			error?: { code: string };
		};
	...
	it("close --all cleans sessions", async () => {
		...
		assert.equal(
			closeResult.code,
			0,
			`close --all failed: ${closeResult.stderr}`,
		);
