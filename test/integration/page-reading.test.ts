...
describe("page reading", { concurrency: false }, () => {
	...
	it("snapshot --boxes includes viewport bounding boxes and refs", async () => {
		const name = makeSessionName();
		sessionsToClean.push(name);
		...
		await runPw([
			...
			"--session",
			name,
			"async page => { await page.setContent(`<main><button>Boxed Action</button></main>`); }",
			"--output",
			"json",
		]);
		...
		assert.equal(result.code, 0, `snapshot --boxes failed: ${result.stderr}`);
		const json = result.json as {
			ok: boolean;
			data: { boxes: boolean; snapshot: string };
		};
		assert.equal(json.ok, true);
		assert.equal(json.data.boxes, true);
		assert.match(json.data.snapshot, /\[ref=e\d+\]/);
		assert.match(json.data.snapshot, /\[box=\d+,\d+,\d+,\d+\]/);
	});
	...
	it("locate --return-ref returns refs for checked controls", async () => {
		...
		assert.equal(
			result.code,
			0,
			`locate --return-ref failed: ${result.stderr}`,
		);
		const json = result.json as {
			ok: boolean;
		...
		assert.equal(json.ok, true);
		assert.ok(
			json.data.candidates.some((candidate) =>
				candidate.text.includes("Remember me"),
			),
		);
	...
	it("tab select and close use stable pageId", async () => {
		...
		assert.equal(
			selectResult.code,
			0,
			`tab select failed: ${selectResult.stderr}`,
		);
		...
		assert.equal(
			closeResult.code,
			0,
			`tab close failed: ${closeResult.stderr}`,
		);
