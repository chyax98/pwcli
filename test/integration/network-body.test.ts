...
describe("network --include-body", { concurrency: false }, () => {
	...
	it("default network does not include full body fields", async () => {
		...
		assert.equal(
			responseRecord.requestBody,
			undefined,
			"default should not include requestBody",
		);
		assert.equal(
			responseRecord.responseBody,
			undefined,
			"default should not include responseBody",
		);
	...
	it("--include-body returns requestBody and responseBody", async () => {
		...
		assert.equal(
			result.code,
			0,
			`network --include-body failed: ${result.stderr}`,
		);
		const json = result.json as {
			ok: boolean;
			data: {
		...
		assert.ok(
			responseRecord.responseBody.length >
				responseRecord.responseBodySnippet?.length,
