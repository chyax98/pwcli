...
describe("interaction", { concurrency: false }, () => {
	...
	it("wait --state hidden honors selector state", async () => {
		...
		const json = result.json as {
			...
			data: {
				matched: boolean;
				condition: { kind: string; selector: string; state: string };
			};
		};
		assert.equal(json.ok, true);
	...
	it("role locator supports accessible description", async () => {
		const name = makeSessionName();
		sessionsToClean.push(name);
		...
		await runPw([
			...
			"--session",
			name,
			'async page => { await page.setContent(`<button aria-description="unique desc">Save</button>`); }',
			"--output",
			"json",
		]);
		...
		assert.equal(
			result.code,
			0,
			`click --description failed: ${result.stderr}`,
		);
		const json = result.json as {
			ok: boolean;
			data: { acted: boolean; target: { description?: string } };
		};
		assert.equal(json.ok, true);
		assert.equal(json.data.acted, true);
		assert.equal(json.data.target.description, "unique desc");
	});
	...
	it("direct action commands cover keyboard, pointer, viewport, file, drag, and download flows", async () => {
        ...
        await page.setContent(\`
          ...
          <script>
            ...
            dropTarget.addEventListener('drop', event => {
              ...
              window.dropPayload = Array.from(event.dataTransfer.types)
                .map(type => type + '=' + event.dataTransfer.getData(type))
                .join('|');
            });
          </script>
        \`);
		...
		const pressJson = pressResult.json as {
			ok: boolean;
			data: { pressed: boolean; key: string };
		};
		...
		const hoverJson = hoverResult.json as {
			ok: boolean;
			data: { acted: boolean };
		};
		...
		const uploadJson = uploadResult.json as {
			...
			data: {
				uploaded: boolean;
				settle: { fileCount: number; settled: boolean };
			};
		};
		...
		const dragJson = dragResult.json as {
			ok: boolean;
		...
		const dropResult = await runPw([
			"drop",
			"--selector",
			"#dropTarget",
			"--data",
			"text/plain=drop-marker",
			"--session",
			name,
			"--output",
			"json",
		]);
		assert.equal(dropResult.code, 0, `drop failed: ${dropResult.stderr}`);
		const dropJson = dropResult.json as {
			ok: boolean;
			data: { dropped: boolean; dataTypes: string[] };
		};
		assert.equal(dropJson.ok, true);
		assert.equal(dropJson.data.dropped, true);
		assert.deepEqual(dropJson.data.dataTypes, ["text/plain"]);
		const dropState = await runPw([
			"code",
			"--session",
			name,
			"async page => await page.evaluate(() => window.dropPayload)",
			"--output",
			"json",
		]);
		assert.equal(
			dropState.code,
			0,
			`drop state read failed: ${dropState.stderr}`,
		);
		assert.equal(
			(dropState.json as { data: { result: string } }).data.result,
			"text/plain=drop-marker",
		);
