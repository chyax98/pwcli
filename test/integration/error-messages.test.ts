...
try {
	...
	const sessionRecreate = await runPw(
		["session", "recreate", "ghost", "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	const code = await runPw(
		["code", "while(true){}", "--session", "ghost", "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	...
	const mouseEnvelope = mouseClick.json as {
		ok: false;
		error: { code: string };
	};
	...
	const accessibility = await runPw(
		["accessibility", "--session", "ghost", "--output", "json"],
		{
			cwd: workspaceDir,
		},
	);
	assert.notEqual(accessibility.code, 0, "accessibility should fail");
	const accessibilityEnvelope = accessibility.json as {
		ok: false;
		error: { code: string };
	};
	...
	const harEnvelope = harReplay.json as {
		ok: false;
