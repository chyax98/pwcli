...
async function expectHelp(args, keywords) {
	...
	assert.equal(
		result.code,
		0,
		`${args.join(" ")} --help failed: ${result.stderr}`,
	);
...
await expectHelp(
	["stream"],
	["Purpose:", "Examples:", "Notes:", "start", "status", "stop"],
);

await expectHelp(
	["view"],
	["Purpose:", "Examples:", "Notes:", "open", "status", "close"],
);

await expectHelp(
	["control-state"],
	["Purpose:", "Examples:", "Notes:", "--session"],
);

await expectHelp(
	["takeover"],
	["Purpose:", "Examples:", "Notes:", "--actor", "--reason"],
);
...
assert.equal(
	authInfoJson.data?.source,
	undefined,
	"auth info must omit source by default",
);
assert.ok(
	Array.isArray(authInfoJson.data?.args),
	"auth info must expose provider args",
);
assert.ok(
	Array.isArray(authInfoJson.data?.examples),
	"auth info must expose provider examples",
);

const authInfoVerbose = await runPw([
	"auth",
	"info",
	"dc",
	"--verbose",
...
assert.equal(
	authInfoVerbose.code,
	0,
	`auth info dc --verbose failed: ${authInfoVerbose.stderr}`,
);
