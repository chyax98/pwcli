...
try {
	...
	if (!status.data.ok || status.data.data?.summary?.pageCount !== 1) {
		throw new Error(
			`status did not recover after RUN_CODE_TIMEOUT: ${status.result.stdout}`,
		);
	}
	...
	if (!clicked.data.ok) {
		throw new Error(
			`click did not recover after RUN_CODE_TIMEOUT: ${clicked.result.stdout}`,
		);
	}
	...
	if (!verified.data.ok || String(verified.data.data?.result) !== "1") {
		throw new Error(
			`short code did not recover after RUN_CODE_TIMEOUT: ${verified.result.stdout}`,
		);
	}
} finally {
