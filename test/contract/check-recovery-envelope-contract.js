...
try {
	const missingSession = await runPwJson([
		"session",
		"status",
		"missing-xyz",
		"--output",
	...
	if (missingSession.code === 0) {
		throw new Error(
			`expected missing session status to fail: ${missingSession.stdout}`,
		);
	}
	if (
		...
		throw new Error(
			`unexpected SESSION_NOT_FOUND envelope: ${missingSession.stdout}`,
		);
	}
	...
	if (tooLongSession.code === 0) {
		throw new Error(
			`expected long session name to fail: ${tooLongSession.stdout}`,
		);
	}
	if (
		...
		throw new Error(
			`unexpected SESSION_NAME_TOO_LONG envelope: ${tooLongSession.stdout}`,
		);
	}
} finally {
