...
try {
	...
	const batch = runPwSync(
		["batch", "--output", "json", "--session", session, "--stdin-json"],
		{
	...
	if (payload.ok !== false || payload.error?.code !== "BATCH_STEP_FAILED") {
		throw new Error(
			`unexpected batch failure payload: ${JSON.stringify(payload, null, 2)}`,
		);
	}
	const summary = payload.error?.details?.summary;
	if (summary?.failedCount !== 1 || summary?.successCount !== 0) {
		throw new Error(
			`unexpected batch summary: ${JSON.stringify(summary, null, 2)}`,
		);
	}
} finally {
	runPwSync(["session", "close", session]);
