...
const resultsPath = resolve(
	repoRoot,
	"test/contract/product-surface-evaluation-results.md",
);
...
function writeResult(surface, result) {
	...
	const previousSection =
		startIndex !== -1 && endIndex !== -1
			? current.slice(startIndex, endIndex + end.length)
			: "";
	const nextSection = sectionFor(
		surface,
		result,
		manualSectionFrom(previousSection),
	);
...
async function main() {
	...
	for (const args of surface.checks) {
		...
		process.stdout.write(
			`[surface ${key}] exit=${result.code} duration=${result.durationMs}ms\n`,
		);
		if (result.code !== 0) {
	...
	process.stdout.write(
		`surface ${key} passed; results written to ${resultsPath}\n`,
	);
}
