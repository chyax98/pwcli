...
try {
	...
	if (
		!clicked.data.ok ||
		clicked.data.data?.blockedState !== "MODAL_STATE_BLOCKED"
	) {
		throw new Error(
			`click did not surface modal blocked state: ${clicked.result.stdout}`,
		);
	}
	...
	if (
		pageCurrent.data.ok ||
		pageCurrent.data.error?.code !== "MODAL_STATE_BLOCKED"
	) {
		throw new Error(
	...
	if (
		...
		throw new Error(
			`doctor did not report modal-state recovery: ${doctor.result.stdout}`,
		);
	}
  ...
  `);
	...
	if (!recreatedConfirm.data.ok)
		throw new Error(
			`session recreate confirm failed: ${recreatedConfirm.result.stdout}`,
		);
	const confirmClick = runPwJsonSync([
		"click",
		"-s",
		session,
		"--selector",
	...
	if (
		!confirmClick.data.ok ||
		confirmClick.data.data?.blockedState !== "MODAL_STATE_BLOCKED"
	) {
		throw new Error(
	...
	if (!confirmDismiss.data.ok || confirmDismiss.data.data?.handled !== true) {
		throw new Error(
			`confirm dialog dismiss failed: ${confirmDismiss.result.stdout}`,
		);
	}
	const confirmResult = runPwJsonSync([
		"get",
		"text",
		"-s",
		session,
	...
	if (confirmResult.data.data?.value !== "no") {
		throw new Error(
			`confirm dismiss result mismatch: ${confirmResult.result.stdout}`,
		);
	}
  ...
  `);
	...
	if (!recreatedPrompt.data.ok)
		throw new Error(
			`session recreate prompt failed: ${recreatedPrompt.result.stdout}`,
		);
	const promptClick = runPwJsonSync([
		"click",
		"-s",
		session,
		"--selector",
	...
	if (
		!promptClick.data.ok ||
		promptClick.data.data?.blockedState !== "MODAL_STATE_BLOCKED"
	) {
		throw new Error(
			`prompt click did not surface modal blocked state: ${promptClick.result.stdout}`,
		);
	}
	const promptAccept = runPwJsonSync([
		"dialog",
		"accept",
		"Codex",
		"-s",
	...
	if (
		...
		throw new Error(
			`prompt dialog accept failed: ${promptAccept.result.stdout}`,
		);
	}
	const promptResult = runPwJsonSync([
		"get",
		"text",
		"-s",
		session,
	...
	if (promptResult.data.data?.value !== "Codex") {
		throw new Error(
			`prompt accept result mismatch: ${promptResult.result.stdout}`,
		);
	}
  ...
  `);
	...
	if (!recreatedHtmlModal.data.ok)
		throw new Error(
			`session recreate html modal failed: ${recreatedHtmlModal.result.stdout}`,
		);
	const openedModal = runPwJsonSync([
		"click",
		"-s",
		session,
		"--selector",
	...
	if (!openedModal.data.ok)
		throw new Error(`html modal open failed: ${openedModal.result.stdout}`);
	...
	if (
		...
		throw new Error(
			`doctor did not report html-modal recovery: ${htmlDoctor.result.stdout}`,
		);
	}
	const closedModal = runPwJsonSync([
		"click",
		"-s",
		session,
		"--selector",
	...
	if (
		(recoveredDoctor.data.data?.diagnostics ?? []).some(
			(item) => item.kind === "html-modal",
		)
	) {
		throw new Error(
