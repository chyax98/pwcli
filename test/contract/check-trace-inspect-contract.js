...
try {
	...
	if (click.status !== 0) {
		throw new Error(
			`trace target click failed\n${click.stdout}\n${click.stderr}`,
		);
	}

	const stop = runPwSync([
		"trace",
		"stop",
		"--session",
		session,
	...
	if (
		inspectPayload.ok !== true ||
		inspectPayload.data?.section !== "actions"
	) {
		throw new Error(
			`unexpected trace inspect payload: ${JSON.stringify(inspectPayload, null, 2)}`,
		);
	}
	...
	const unsupportedFilterPayload = parseJson(
		unsupportedFilter.stdout,
		"trace inspect filter",
	);
	if (
