...
try {
	...
	if (typeof root !== "string" || !root.endsWith("/skills/pwcli")) {
		throw new Error(
			`unexpected skill path: ${JSON.stringify(pathPayload, null, 2)}`,
		);
	}
	if (
		pathPayload.data?.info?.exists !== true ||
		!existsSync(join(root, "SKILL.md"))
	) {
		throw new Error(
			`packaged skill root is not usable: ${JSON.stringify(pathPayload, null, 2)}`,
		);
	}

	const install = runPwSync([
		"skill",
		"install",
		targetParent,
		"--output",
	...
	if (installPayload.data?.installed !== true) {
		throw new Error(
			`unexpected skill install payload: ${JSON.stringify(installPayload, null, 2)}`,
		);
	}
