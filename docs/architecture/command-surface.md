...
| MCP | `mcp schema|serve` | `mcp.ts`、`src/domain/mcp/service.ts`、`src/infra/mcp/server.ts` | 以 thin second surface 暴露高频 pwcli tool contract，保留 CLI 为 authoritative surface | `command-reference-advanced.md` |
...
MCP 只暴露高频稳定 lanes。它复用 CLI/domain contract，不单独发明第二套行为语义。
